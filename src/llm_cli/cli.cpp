            ...
            if (res_partial) {
                ...
                for (const auto & diff : res_partial->oaicompat_msg_diffs) {
                    if (!diff.content_delta.empty()) {
                        ...
                        lhm::console::log("%s", diff.content_delta.c_str());
                        lhm::console::flush();
                    }
                    if (!diff.reasoning_content_delta.empty()) {
                        lhm::console::set_display(DISPLAY_TYPE_REASONING);
                        if (!is_thinking) {
                            lhm::console::log("[Start thinking]\n");
                        }
                        is_thinking = true;
                        lhm::console::log("%s", diff.reasoning_content_delta.c_str());
                        lhm::console::flush();
                    }
                }
            }
...
int llm_cli() {
    ...
    if (params.conversation_mode == COMMON_CONVERSATION_MODE_DISABLED) {
        lhm::console::error("--no-conversation is not supported by llm_cli\n");
        lhm::console::error("please use llm_completion instead\n");
    }
    ...
    atexit([]() { lhm::console::cleanup(); });

    lhm::console::set_display(DISPLAY_TYPE_RESET);
    lhm::console::set_completion_callback(auto_completion_callback);
    ...
    lhm::console::log("\n");
    lhm::console::log("available commands:\n");
    lhm::console::log("  /exit or Ctrl+C     stop or exit\n");
    lhm::console::log("  /regen              regenerate the last response\n");
    ...
    while (true) {
        std::string buffer;
        lhm::console::set_display(DISPLAY_TYPE_USER_INPUT);
        if (params.prompt.empty()) {
            lhm::console::log("\n> ");
            std::string line;
            bool another_line = true;
            do {
                another_line = lhm::console::readline(line, params.multiline_input);
                buffer += line;
            } while (another_line);
        } else {
