...
static std::string server_state_to_str(server_state state) {
    switch (state) {
        ...
        default: LHM_ASSERT("invalid server_state");
    }
}
...
static server_state server_state_from_str(const std::string & str) {
    ...
    LHM_ASSERT("invalid server_state string");
}
...
struct server_context_meta {
    ...
    lhm_token fim_pre_token;
    lhm_token fim_sub_token;
    lhm_token fim_mid_token;
    lhm_token fim_pad_token;
    lhm_token fim_rep_token;
    lhm_token fim_sep_token;

    // sampling
    std::vector<lhm_logit_bias> logit_bias_eog;

    // model meta
    enum lhm_vocab_type model_vocab_type;
...
struct server_context {
    ...
    // get the underlaying lhm_context, can return nullptr if sleeping
    // not thread-safe, should only be used from the main thread
    lhm_context * get_lhm_context() const;
