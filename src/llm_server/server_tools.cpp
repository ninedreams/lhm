            ...
            {"function", {
                ...
                {"parameters", {
                    ...
                    {"properties", {
                        ...
                        {"timeout",         {{"type", "integer"}, {"description", fmt::format("Timeout in seconds (default 10, max %d)", SERVER_TOOL_EXEC_SHELL_COMMAND_MAX_TIMEOUT)}}},
    ...
    json invoke(json params) const override {
        ...
        for (const auto & e : edits_json) {
            ...
            if (er.old_text.empty()) {
                return {{"error", fmt::format("edits[%zu].old_text must not be empty", edits.size())}};
            }
