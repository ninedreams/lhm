...
std::vector<std::unique_ptr<field>> make_lhm_cmpl_schema(const common_params & params_base, task_params & params) {
        ...
        ->set_handler([&](field_eval_context & ctx, const json & data) {
            ...
            for (const auto & t : data.at("grammar_triggers")) {
                ...
                if (ct.value.type == COMMON_GRAMMAR_TRIGGER_TYPE_WORD) {
                    ...
                    if (ids.size() == 1) {
                        auto token = ids[0];
                        if (std::find(ctx.params.sampling.preserved_tokens.begin(), ctx.params.sampling.preserved_tokens.end(), (lhm_token) token) == ctx.params.sampling.preserved_tokens.end()) {
            ...
            if (logit_bias.is_array()) {
                for (const auto & el : logit_bias) {
                    ...
                    if (el[0].is_number_integer()) {
                        lhm_token tok = el[0].get<lhm_token>();
                ...
                for (const auto & el : logit_bias.items()) {
                    ...
                    char * end;
                    lhm_token tok = strtol(el.key().c_str(), &end, 10);
                    if (*end == 0) {
                        if (tok >= 0 && tok < n_vocab) ctx.params.sampling.logit_bias.push_back({tok, bias});
                    } else {
...
task_params eval_lhm_cmpl_schema(
                const lhm_vocab * vocab,
                const common_params & params_base,
                const int n_ctx_slot,
                const std::vector<lhm_logit_bias> & logit_bias_eog,
                const json & data) {
    task_params params;
    ...
    auto schema = make_lhm_cmpl_schema(params_base, params);
