...
struct server_grammar_trigger {
    ...
    server_grammar_trigger(const json & in) {
        ...
        if (value.type == COMMON_GRAMMAR_TRIGGER_TYPE_TOKEN) {
            value.token = (lhm_token) in.at("token").get<int>();
        }
    }
...
public:
    ...
    lhm_pos pos_next(int64_t n_tokens = -1) const;
    ...
    void insert(const lhm_tokens & inp_tokens);

    // for compatibility with speculative decoding, ctx shift, slot save/load
    const lhm_tokens & get_tokens() const;

    lhm_tokens get_text_tokens() const;

    // for compatibility with speculative decoding
    void set_token(lhm_pos pos, lhm_token id);
    ...
    std::string detokenize(const lhm_context * ctx, bool special) const;

    size_t get_common_prefix(const server_tokens & b) const;

    // make sure all text tokens are within the vocab range
    bool validate(const struct lhm_context * ctx) const;
...
// TODO: move it to server_task.cpp
...
std::vector<lhm_token_data> get_token_probabilities(lhm_context * ctx, int idx);
...
// TODO: move these to server_task.cpp
//
