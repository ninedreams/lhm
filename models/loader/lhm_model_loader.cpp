...
const char * lhm_file_version_name(lhm_fver version) {
    switch (version) {
...
static std::string lhm_model_ftype_name(lhm_ftype ftype) {
    if (ftype & LHM_FTYPE_GUESSED) {
        return lhm_model_ftype_name((enum lhm_ftype) (ftype & ~LHM_FTYPE_GUESSED)) + " (guessed)";
    }

    switch (ftype) {
        case LHM_FTYPE_ALL_F32:         return "all F32";
        case LHM_FTYPE_MOSTLY_F16:      return "F16";
        case LHM_FTYPE_MOSTLY_BF16:     return "BF16";
        case LHM_FTYPE_MOSTLY_Q1_0:     return "Q1_0";
        case LHM_FTYPE_MOSTLY_Q4_0:     return "Q4_0";
        case LHM_FTYPE_MOSTLY_Q4_1:     return "Q4_1";
        case LHM_FTYPE_MOSTLY_Q5_0:     return "Q5_0";
        case LHM_FTYPE_MOSTLY_Q5_1:     return "Q5_1";
        case LHM_FTYPE_MOSTLY_Q8_0:     return "Q8_0";
        case LHM_FTYPE_MOSTLY_MXFP4_MOE: return "MXFP4 MoE";
        case LHM_FTYPE_MOSTLY_NVFP4:    return "NVFP4";
        case LHM_FTYPE_MOSTLY_Q2_K:     return "Q2_K - Medium";
        case LHM_FTYPE_MOSTLY_Q2_K_S:   return "Q2_K - Small";
        case LHM_FTYPE_MOSTLY_Q3_K_S:   return "Q3_K - Small";
        case LHM_FTYPE_MOSTLY_Q3_K_M:   return "Q3_K - Medium";
        case LHM_FTYPE_MOSTLY_Q3_K_L:   return "Q3_K - Large";
        case LHM_FTYPE_MOSTLY_Q4_K_S:   return "Q4_K - Small";
        case LHM_FTYPE_MOSTLY_Q4_K_M:   return "Q4_K - Medium";
        case LHM_FTYPE_MOSTLY_Q5_K_S:   return "Q5_K - Small";
        case LHM_FTYPE_MOSTLY_Q5_K_M:   return "Q5_K - Medium";
        case LHM_FTYPE_MOSTLY_Q6_K:     return "Q6_K";
        case LHM_FTYPE_MOSTLY_TQ1_0:    return "TQ1_0 - 1.69 bpw ternary";
        case LHM_FTYPE_MOSTLY_TQ2_0:    return "TQ2_0 - 2.06 bpw ternary";
        case LHM_FTYPE_MOSTLY_IQ2_XXS:  return "IQ2_XXS - 2.0625 bpw";
        case LHM_FTYPE_MOSTLY_IQ2_XS:   return "IQ2_XS - 2.3125 bpw";
        case LHM_FTYPE_MOSTLY_IQ2_S:    return "IQ2_S - 2.5 bpw";
        case LHM_FTYPE_MOSTLY_IQ2_M:    return "IQ2_M - 2.7 bpw";
        case LHM_FTYPE_MOSTLY_IQ3_XS:   return "IQ3_XS - 3.3 bpw";
        case LHM_FTYPE_MOSTLY_IQ3_XXS:  return "IQ3_XXS - 3.0625 bpw";
        case LHM_FTYPE_MOSTLY_IQ1_S:    return "IQ1_S - 1.5625 bpw";
        case LHM_FTYPE_MOSTLY_IQ1_M:    return "IQ1_M - 1.75 bpw";
        case LHM_FTYPE_MOSTLY_IQ4_NL:   return "IQ4_NL - 4.5 bpw";
        case LHM_FTYPE_MOSTLY_IQ4_XS:   return "IQ4_XS - 4.25 bpw";
        case LHM_FTYPE_MOSTLY_IQ3_S:    return "IQ3_S - 3.4375 bpw";
        case LHM_FTYPE_MOSTLY_IQ3_M:    return "IQ3_S mix - 3.66 bpw";
...
static std::vector<std::string> lhm_get_list_splits(const std::string & path, const int idx, const int n_split) {
    std::vector<std::string> paths;
    std::string split_prefix;
    std::vector<char> buf(lhm_path_max(), 0);
    ...
    for (int idx = 0; idx < n_split; ++idx) {
        int ret = lhm_split_path(buf.data(), buf.size(), split_prefix.c_str(), idx, n_split);
...
namespace GGUFMeta {
    ...
    class GKV : public GKV_Base<T> {
        ...
        static const char * override_type_to_str(const lhm_model_kv_override_type ty) {
            switch (ty) {
                case LHM_KV_OVERRIDE_TYPE_BOOL:  return "bool";
                case LHM_KV_OVERRIDE_TYPE_INT:   return "int";
                case LHM_KV_OVERRIDE_TYPE_FLOAT: return "float";
                case LHM_KV_OVERRIDE_TYPE_STR:   return "str";
            }
            return "unknown";
        }
        ...
        static bool validate_override(const lhm_model_kv_override_type expected_type, const struct lhm_model_kv_override * ovrd) {
            ...
            if (ovrd->tag == expected_type) {
                std::string log_msg = fmt::format("Using metadata override ({:5s}) '{}' = ", override_type_to_str(ovrd->tag), ovrd->key);
                switch (ovrd->tag) {
                    case LHM_KV_OVERRIDE_TYPE_BOOL:  {
                        LOG_INFO("{}{}", log_msg, ovrd->val_bool ? "true" : "false");
                    } break;
                    case LHM_KV_OVERRIDE_TYPE_INT:   {
                        LOG_INFO("{}{}", log_msg, ovrd->val_i64);
                    } break;
                    case LHM_KV_OVERRIDE_TYPE_FLOAT: {
                        LOG_INFO("{}{:.6f}", log_msg, ovrd->val_f64);
                    } break;
                    case LHM_KV_OVERRIDE_TYPE_STR: {
                        LOG_INFO("{}{}", log_msg, ovrd->val_str);
                    } break;
                    default:
                        // Shouldn't be possible to end up here, but just in case...
                        throw std::runtime_error(
                            fmt::format("Unsupported attempt to override {} type for metadata key {}",
        ...
        try_override(OT & target, const struct lhm_model_kv_override * ovrd) {
            if (validate_override(LHM_KV_OVERRIDE_TYPE_BOOL, ovrd)) {
        ...
        try_override(OT & target, const struct lhm_model_kv_override * ovrd) {
            if (validate_override(LHM_KV_OVERRIDE_TYPE_INT, ovrd)) {
                target = ovrd->val_i64;
                return true;
            }
            return false;
        ...
        try_override(T & target, const struct lhm_model_kv_override * ovrd) {
            if (validate_override(LHM_KV_OVERRIDE_TYPE_FLOAT, ovrd)) {
                target = ovrd->val_f64;
                return true;
            }
            return false;
        ...
        try_override(T & target, const struct lhm_model_kv_override * ovrd) {
            if (validate_override(LHM_KV_OVERRIDE_TYPE_STR, ovrd)) {
                target = ovrd->val_str;
                return true;
            }
            return false;
        ...
        static bool set(const gguf_context * ctx, const int k, T & target, const struct lhm_model_kv_override * ovrd = nullptr) {
        ...
        static bool set(const gguf_context * ctx, const char * key, T & target, const struct lhm_model_kv_override * ovrd = nullptr) {
            return set(ctx, gguf_find_key(ctx, key), target, ovrd);
        }

        static bool set(const gguf_context * ctx, const std::string & key, T & target, const struct lhm_model_kv_override * ovrd = nullptr) {
    ...
    lhm_model_loader::get_arr_n(const std::string & key, T & result, bool required) {
    ...
    template<typename T>
    typename std::enable_if<std::is_integral<T>::value, bool>::type
    lhm_model_loader::get_arr_n(enum llm_kv kid, T & result, bool required) {
        return get_arr_n(llm_kv(kid), result, required);
    }

    template bool lhm_model_loader::get_arr_n(enum llm_kv kid, uint32_t & result, bool required);
    template std::enable_if<std::is_integral<uint32_t>::value, bool>::type
    lhm_model_loader::get_arr_n<uint32_t>(const std::string & key, uint32_t & result, bool required);

    template<typename T>
    bool lhm_model_loader::get_arr(const std::string & key, std::vector<T> & result, bool required) {
    ...
    bool lhm_model_loader::get_arr(const std::string & key, std::array<T, N_MAX> & result, bool required) {
        const gguf_context * ctx = metadata;
        const int kid = gguf_find_key(ctx, key.c_str());
    ...
    template<typename T>
    bool lhm_model_loader::get_arr(enum llm_kv kid, T & result, bool required) {
        return get_arr(llm_kv(kid), result, required);
    }

    template bool lhm_model_loader::get_arr<std::vector<std::string>>(enum llm_kv kid, std::vector<std::string> & result, bool required);
    template bool lhm_model_loader::get_arr<std::array<int32_t, 512>>(enum llm_kv kid, std::array<int32_t, 512> & result, bool required);
    template bool lhm_model_loader::get_arr<std::vector<int32_t>>(enum llm_kv kid, std::vector<int32_t> & result, bool required);
    template bool lhm_model_loader::get_arr<std::array<uint32_t, LHM_MAX_LAYERS>>(enum llm_kv kid, std::array<uint32_t, LHM_MAX_LAYERS> & result, bool required);

    template<typename T>
    bool lhm_model_loader::get_key(const std::string & key, T & result, bool required) {
        auto it = kv_overrides.find(key);

        const struct lhm_model_kv_override * override =
    ...
    template<typename T>
    bool lhm_model_loader::get_key(enum llm_kv kid, T & result, bool required) {
        return get_key(llm_kv(kid), result, required);
    }

    template bool lhm_model_loader::get_key<bool>       (enum llm_kv kid, bool & result,        bool required);
    template bool lhm_model_loader::get_key<float>      (enum llm_kv kid, float & result,       bool required);
    template bool lhm_model_loader::get_key<uint32_t>   (enum llm_kv kid, uint32_t & result,    bool required);
    template bool lhm_model_loader::get_key<std::string>(enum llm_kv kid, std::string & result, bool required);

    template<>
    bool lhm_model_loader::get_key(enum llm_kv kid, enum lhm_pooling_type & result, bool required) {
        uint32_t tmp;
        const bool found = get_key(kid, tmp, required);
        if (found) {
            result = (enum lhm_pooling_type) tmp;
        } else {
            result = LHM_POOLING_TYPE_UNSPECIFIED;
        }
        return found;
    }
    ...
    template<typename T, size_t N_MAX>
    bool lhm_model_loader::get_key_or_arr(const std::string & key, std::array<T, N_MAX> & result, uint32_t n, bool required) {
        const int kid = gguf_find_key(metadata, key.c_str());
    ...
    template<typename T>
    bool lhm_model_loader::get_key_or_arr(enum llm_kv kid, T & result, uint32_t n, bool required) {
        return get_key_or_arr(llm_kv(kid), result, n, required);
    }

    bool lhm_model_loader::get_key_or_arr(enum llm_kv kid, uint32_t & result, bool required) {
    ...
    template bool lhm_model_loader::get_key_or_arr<std::array<int,      4>>  (enum llm_kv kid, std::array<int,      4>   & result, uint32_t n, bool required);
    template bool lhm_model_loader::get_key_or_arr<std::array<uint32_t, 512>>(enum llm_kv kid, std::array<uint32_t, 512> & result, uint32_t n, bool required);
    template bool lhm_model_loader::get_key_or_arr<std::array<float,    512>>(enum llm_kv kid, std::array<float,    512> & result, uint32_t n, bool required);


lhm_model_loader::lhm_model_loader(
        ...
        bool check_tensors,
        bool no_alloc,
        const lhm_model_kv_override * param_overrides_p,
        const lhm_model_tensor_buft_override * param_tensor_buft_overrides_p)
        : metadata(meta), set_tensor_data(set_tensor_data), set_tensor_data_ud(set_tensor_data_ud) {
    int trace = 0;
    if (getenv("LHM_TRACE")) {
        trace = atoi(getenv("LHM_TRACE"));
    }

    if (param_overrides_p != nullptr) {
        for (const struct lhm_model_kv_override * p = param_overrides_p; p->key[0] != 0; p++) {
    ...
    if (!fname.empty()) {
        ...
        files.emplace_back(new lhm_file(fname.c_str(), "rb", use_direct_io));
        ...
        if (use_mmap && use_direct_io) {
            if (files.back()->has_direct_io()) {
                ...
                files.pop_back();
                files.emplace_back(new lhm_file(fname.c_str(), "rb", false));
            }
        }
        ...
        for (ggml_tensor * cur = ggml_get_first_tensor(ctx); cur; cur = ggml_get_next_tensor(ctx, cur)) {
            ...
            weights_map.emplace(tensor_name, lhm_tensor_weight(files.back().get(), 0, metadata, cur));
        }
        uint16_t n_split = 0;
        ...
        if (n_split > 1) {
            ...
            if (splits.empty()) {
                splits = lhm_get_list_splits(fname, idx, n_split);
            }
            ...
            for (idx = 1; idx < n_split; idx++) {
                ...
                files.emplace_back(new lhm_file(fname_split, "rb", use_direct_io));
                contexts.emplace_back(ctx);
                ...
                for (ggml_tensor * cur = ggml_get_first_tensor(ctx); cur; cur = ggml_get_next_tensor(ctx, cur)) {
                    ...
                    n_elements += ggml_nelements(cur);
                    n_bytes    += ggml_nbytes(cur);
                    weights_map.emplace(tensor_name, lhm_tensor_weight(files.back().get(), idx, ctx_gguf.get(), cur));
                }
            }
        ...
        files.emplace_back(new lhm_file(file));
        contexts.emplace_back(ctx);
    ...
    // TODO: make optional
    {
        ...
        for (const auto & it : weights_map) {
            const lhm_tensor_weight & w = it.second;
        ...
        switch (type_max) {
            case GGML_TYPE_F32:     ftype = LHM_FTYPE_ALL_F32;        break;
            case GGML_TYPE_F16:     ftype = LHM_FTYPE_MOSTLY_F16;     break;
            case GGML_TYPE_BF16:    ftype = LHM_FTYPE_MOSTLY_BF16;    break;
            case GGML_TYPE_Q4_0:    ftype = LHM_FTYPE_MOSTLY_Q4_0;    break;
            case GGML_TYPE_Q4_1:    ftype = LHM_FTYPE_MOSTLY_Q4_1;    break;
            case GGML_TYPE_Q5_0:    ftype = LHM_FTYPE_MOSTLY_Q5_0;    break;
            case GGML_TYPE_Q5_1:    ftype = LHM_FTYPE_MOSTLY_Q5_1;    break;
            case GGML_TYPE_Q8_0:    ftype = LHM_FTYPE_MOSTLY_Q8_0;    break;
            case GGML_TYPE_Q2_K:    ftype = LHM_FTYPE_MOSTLY_Q2_K;    break;
            case GGML_TYPE_Q3_K:    ftype = LHM_FTYPE_MOSTLY_Q3_K_M;  break;
            case GGML_TYPE_Q4_K:    ftype = LHM_FTYPE_MOSTLY_Q4_K_M;  break;
            case GGML_TYPE_Q5_K:    ftype = LHM_FTYPE_MOSTLY_Q5_K_M;  break;
            case GGML_TYPE_Q6_K:    ftype = LHM_FTYPE_MOSTLY_Q6_K;    break;
            case GGML_TYPE_TQ1_0:   ftype = LHM_FTYPE_MOSTLY_TQ1_0;   break;
            case GGML_TYPE_TQ2_0:   ftype = LHM_FTYPE_MOSTLY_TQ2_0;   break;
            case GGML_TYPE_IQ2_XXS: ftype = LHM_FTYPE_MOSTLY_IQ2_XXS; break;
            case GGML_TYPE_IQ2_XS:  ftype = LHM_FTYPE_MOSTLY_IQ2_XS;  break;
            case GGML_TYPE_IQ2_S:   ftype = LHM_FTYPE_MOSTLY_IQ2_S;   break;
            case GGML_TYPE_IQ3_XXS: ftype = LHM_FTYPE_MOSTLY_IQ3_XXS; break;
            case GGML_TYPE_IQ1_S:   ftype = LHM_FTYPE_MOSTLY_IQ1_S;   break;
            case GGML_TYPE_IQ1_M:   ftype = LHM_FTYPE_MOSTLY_IQ1_M;   break;
            case GGML_TYPE_IQ4_NL:  ftype = LHM_FTYPE_MOSTLY_IQ4_NL;  break;
            case GGML_TYPE_IQ4_XS:  ftype = LHM_FTYPE_MOSTLY_IQ4_XS;  break;
            case GGML_TYPE_IQ3_S:   ftype = LHM_FTYPE_MOSTLY_IQ3_S;   break;
            case GGML_TYPE_NVFP4:   ftype = LHM_FTYPE_MOSTLY_NVFP4;   break;
        ...
        for (int i = 0; i < n_kv; i++) {
            ...
            const std::string type_name =
                ...
                ? fmt::format("{}[{},{}]", gguf_type_name(type), gguf_type_name(gguf_get_arr_type(metadata, i)), gguf_get_arr_n(metadata, i))
                : gguf_type_name(type);
...
std::string lhm_model_loader::get_arch_name() const {
    return arch_name;
}

enum llm_arch lhm_model_loader::get_arch() const {
    return llm_kv.arch;
}

const lhm_model_loader::lhm_tensor_weight * lhm_model_loader::get_weight(const char * name) const {
...
struct ggml_tensor * lhm_model_loader::get_tensor_meta(const char * name) const {
...
const struct ggml_tensor * lhm_model_loader::check_tensor_dims(const std::string & name, const std::vector<int64_t> & ne, bool required) const {
...
struct ggml_tensor * lhm_model_loader::create_tensor(
        const lhm_hparams & hparams, const buft_list_t * buft_list_cpu, const buft_list_t * buft_list_input, const buft_list_t * buft_list_output,
                ...
                if (std::regex_search(tensor_name, pattern)) {
                    if (overrides->buft == ggml_backend_cpu_buffer_type()) {
                        ...
                        if (use_mmap) {
                            ...
                            std::call_once(once, [] {
                                LOG_WARN("lhm_model_loader: tensor overrides to CPU are used with mmap enabled - consider using --no-mmap for better performance\n");
                            });
                        }
                    } else {
                        buft = overrides->buft;
...
struct ggml_tensor * lhm_model_loader::create_tensor_as_view(struct ggml_context * ctx, struct ggml_tensor * base, const std::string & name, const std::initializer_list<int64_t> & ne, size_t offset, bool required) {
...
void lhm_model_loader::init_mappings(bool prefetch, lhm_mlocks * mlock_mmaps) {
    if (use_mmap) {
        ...
        for (const auto & file : files) {
            ...
            std::unique_ptr<lhm_mmap> mapping = std::make_unique<lhm_mmap>(file.get(), prefetch ? -1 : 0, is_numa);
            mmaps_used.emplace_back(mapping->size(), 0);
            if (mlock_mmaps) {
                std::unique_ptr<lhm_mlock> mlock_mmap(new lhm_mlock());
...
void lhm_model_loader::load_data_for(struct ggml_tensor * cur) const {
...
bool lhm_model_loader::load_all_data(
...
std::string lhm_model_loader::ftype_name() const {
    return lhm_model_ftype_name(ftype);
}
