...
bool lhm_model_saver_supports_arch(llm_arch arch) {
    switch (arch) {
...
lhm_model_saver::~lhm_model_saver() {
    if (gguf_ctx_owned) {
        gguf_free(gguf_ctx);
    }
}

void lhm_model_saver::add_kv(const enum llm_kv key, const uint32_t value) {
    gguf_set_val_u32(gguf_ctx, llm_kv(key).c_str(), value);
}

void lhm_model_saver::add_kv(const enum llm_kv key, const int32_t value) {
    gguf_set_val_i32(gguf_ctx, llm_kv(key).c_str(), value);
}

void lhm_model_saver::add_kv(const enum llm_kv key, const float value) {
    gguf_set_val_f32(gguf_ctx, llm_kv(key).c_str(), value);
}

void lhm_model_saver::add_kv(const enum llm_kv key, const bool value) {
    gguf_set_val_bool(gguf_ctx, llm_kv(key).c_str(), value);
}

void lhm_model_saver::add_kv(const enum llm_kv key, const char * value) {
...
template void lhm_model_saver::add_kv<std::vector<uint32_t>>(const enum llm_kv, const std::vector<uint32_t> &, const bool);

void lhm_model_saver::add_kv(const enum llm_kv key, const std::vector<std::string> & value) {
...
void lhm_model_saver::add_tensor(const struct ggml_tensor * tensor) {
    if (!tensor) {
        return;
    }
...
void lhm_model_saver::add_kv_from_model() {
    const lhm_hparams & hparams = model->hparams;
    const lhm_vocab   & vocab   = model->vocab;
    ...
    if (vocab.get_type() != LHM_VOCAB_TYPE_NONE) {
        for (int32_t id = 0; id < n_vocab; ++id) {
            const lhm_vocab::token_data & token_data = vocab.get_token_data(id);
            ...
            switch(token_data.attr) {
                case LHM_TOKEN_ATTR_UNKNOWN:      token_types[id] = LHM_TOKEN_TYPE_UNKNOWN;      break;
                case LHM_TOKEN_ATTR_UNUSED:       token_types[id] = LHM_TOKEN_TYPE_UNUSED;       break;
                case LHM_TOKEN_ATTR_NORMAL:       token_types[id] = LHM_TOKEN_TYPE_NORMAL;       break;
                case LHM_TOKEN_ATTR_CONTROL:      token_types[id] = LHM_TOKEN_TYPE_CONTROL;      break;
                case LHM_TOKEN_ATTR_USER_DEFINED: token_types[id] = LHM_TOKEN_TYPE_USER_DEFINED; break;
                case LHM_TOKEN_ATTR_BYTE:         token_types[id] = LHM_TOKEN_TYPE_BYTE;         break;
                // case LHM_TOKEN_ATTR_NORMALIZED:   ???
                // case LHM_TOKEN_ATTR_LSTRIP:       ???
                // case LHM_TOKEN_ATTR_RSTRIP:       ???
                case LHM_TOKEN_ATTR_UNDEFINED:
                default:                            token_types[id] = LHM_TOKEN_TYPE_UNDEFINED;    break;
            }
        }
    }
    ...
    add_kv(LLM_KV_ROPE_SCALING_TYPE,                 lhm_rope_scaling_type_name(hparams.rope_scaling_type_train));
    ...
    // FIXME lhm_token is type i32 but when reading in a GGUF file u32 is expected, not an issue for writing though
...
void lhm_model_saver::add_tensors_from_model() {
    ...
    for (const struct lhm_layer & layer : model->layers) {
...
void lhm_model_saver::save(const std::string & path_model) {
    gguf_write_to_file(gguf_ctx, path_model.c_str(), false);
}

void lhm_model_saver::save(FILE * file) {
