...
bool common_chat_verify_template(const std::string & tmpl, bool use_jinja) {
    ...
    lhm_chat_message chat[] = {
        { "user", "test" }
    };
    const int res = lhm_chat_apply_template(tmpl.c_str(), chat, 1, true, nullptr, 0);
    return res >= 0;
}
...
common_chat_templates_ptr common_chat_templates_init(const struct lhm_model * model,
    ...
    if (chat_template_override.empty()) {
        ...
        str = lhm_model_chat_template(model, /* name */ "tool_use");
        if (str) {
    ...
    bool        add_eos   = false;
    if (model) {
        const auto * vocab     = lhm_model_get_vocab(model);
        const auto   get_token = [&](lhm_token token, const char * name, const char * jinja_variable_name) {
        ...
        token_bos = get_token(lhm_vocab_bos(vocab), "BOS", "bos_token");
        token_eos = get_token(lhm_vocab_eos(vocab), "EOS", "eos_token");
        add_bos   = lhm_vocab_get_add_bos(vocab);
        add_eos   = lhm_vocab_get_add_eos(vocab);
    }
...
static void system_message_not_supported(json & messages) {
    if (!messages.empty() && messages.front().at("role") == "system") {
        if (messages.size() > 1) {
            LOG_DEBUG("Merging system prompt into next message\n");
            ...
            LOG_WARN("Removing system prompt due to template not supporting system role\n");
            messages.erase(messages.begin());
        }
    }
}
...
std::optional<common_chat_params> common_chat_try_specialized_template(
    ...
    if (src.find("[SYSTEM_PROMPT]") != std::string::npos && src.find("[TOOL_CALLS]") != std::string::npos &&
        ...
        LOG_DEBUG("Using specialized template: Ministral/Magistral Large 3\n");
        return common_chat_params_init_ministral_3(tmpl, params);
    }

    // GPT-OSS - has unique channel-based structure that needs dedicated handler
    if (src.find("<|channel|>") != std::string::npos) {
        LOG_DEBUG("Using specialized template: GPT-OSS\n");
    ...
    if (src.find(">>>all") != std::string::npos && src.find(">>>${recipient}") != std::string::npos) {
        LOG_DEBUG("Using specialized template: Functionary v3.2\n");
    ...
    if (src.find("<|tool_calls_section_begin|>") != std::string::npos &&
        ...
        LOG_DEBUG("Using specialized template: Kimi K2 Thinking\n");
    ...
    if (src.find("<|START_TEXT|>") != std::string::npos &&
        ...
        LOG_DEBUG("Using specialized template: Cohere2 MoE\n");
        return common_chat_params_init_cohere2moe(tmpl, params);
    }

    if (is_lfm2_template(src)) {
        LOG_DEBUG("Using specialized template: LFM2\n");
    ...
    if (src.find("List of tools: [") != std::string::npos &&
        ...
        LOG_DEBUG("Using specialized template: LFM2.5\n");
    ...
    if (src.find("<|role_sep|>") != std::string::npos &&
        ...
        LOG_DEBUG("Using specialized template: GigaChatV3\n");
    ...
    if (src.find("dsml_token") != std::string::npos &&
        ...
        LOG_DEBUG("Using specialized template: DeepSeek V3.2\n");
...
static common_chat_params common_chat_templates_apply_jinja(const struct common_chat_templates *        tmpls,
    ...
    if (inputs.force_pure_content) {
        LOG_WARN("Forcing pure content template, will not render reasoning or tools separately.");
...
// Legacy template route (adhoc C++ implementation of known templates), forward to lhm_chat_apply_template.
static common_chat_params common_chat_templates_apply_legacy(const struct common_chat_templates *        tmpls,
                                                             const struct common_chat_templates_inputs & inputs) {
    size_t                          alloc_size = 0;
    std::vector<lhm_chat_message> chat;
    ...
    int32_t      res = lhm_chat_apply_template(src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt,
                                                 buf.data(), buf.size());

    // error: chat template is not supported
    if (res < 0) {
        // if the custom "tmpl" is not supported, we throw an error
        // this is a bit redundant (for good), since we're not sure if user validated the custom template with lhm_chat_verify_template()
    ...
    if ((size_t) res > buf.size()) {
        buf.resize(res);
        res = lhm_chat_apply_template(src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt, buf.data(),
                                        buf.size());
    }
...
common_chat_msg common_chat_peg_parse(const common_peg_arena &          src_parser,
    ...
    if (src_parser.empty()) {
        LOG_DEBUG("No parser definition detected, assuming pure content parser.");
    }
