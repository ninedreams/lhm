...
class lhm_kv_cache : public lhm_memory_i {
public:
    ...
    slot_info_vec_t prepare(const std::vector<lhm_ubatch> & ubatches);

    bool update(lhm_context * lctx, bool do_shift, const stream_copy_info & sc_info);

    // find a slot of kv cells that can hold the ubatch
    // if cont == true, then the slot must be continuous
    // return empty slot_info on failure
    slot_info find_slot(const lhm_ubatch & ubatch, bool cont) const;

    // emplace the ubatch context into slot: [sinfo.idxs[0...ubatch.n_tokens - 1]]
    void apply_ubatch(const slot_info & sinfo, const lhm_ubatch & ubatch);
    ...
    ggml_tensor * build_input_v_idxs(ggml_context * ctx, const lhm_ubatch & ubatch) const;

    ggml_tensor * build_input_k_rot(ggml_context * ctx) const;
    ggml_tensor * build_input_v_rot(ggml_context * ctx) const;

    void set_input_k_idxs(ggml_tensor * dst, const lhm_ubatch * ubatch, const slot_info & sinfo) const;
    void set_input_v_idxs(ggml_tensor * dst, const lhm_ubatch * ubatch, const slot_info & sinfo) const;

    void set_input_k_shift(ggml_tensor * dst) const;

    void set_input_kq_mask   (ggml_tensor * dst, const lhm_ubatch * ubatch, bool causal_attn) const;
    void set_input_pos_bucket(ggml_tensor * dst, const lhm_ubatch * ubatch) const;
...
private:
    ...
    // env: LHM_KV_CACHE_DEBUG
    int debug = 0;

    // this is the SWA type of the cache - not to be confused with the model SWA type
    const lhm_swa_type swa_type = LHM_SWA_TYPE_NONE;
    ...
    std::shared_ptr<lhm_kv_cells_vec> v_cells_impl;

    lhm_kv_cells_vec & v_cells;
    ...
    void state_write_meta(lhm_io_write_i & io, const cell_ranges_t & cr, lhm_seq_id seq_id = -1) const;
    void state_write_data(lhm_io_write_i & io, const cell_ranges_t & cr) const;

    bool state_read_meta(lhm_io_read_i & io, uint32_t strm, uint32_t cell_count,       slot_info & sinfo, lhm_seq_id dest_seq_id = -1);
    bool state_read_data(lhm_io_read_i & io, uint32_t strm, uint32_t cell_count, const slot_info & sinfo);
};

class lhm_kv_cache_context : public lhm_memory_context_i {
public:
    // some shorthands
    using slot_info_vec_t  = lhm_kv_cache::slot_info_vec_t;
    using stream_copy_info = lhm_kv_cache::stream_copy_info;

    // used for errors
    lhm_kv_cache_context(lhm_memory_status status);
    ...
    //
    // lhm_kv_cache_context specific API
    //
    ...
    void set_input_k_idxs(ggml_tensor * dst, const lhm_ubatch * ubatch) const;
    void set_input_v_idxs(ggml_tensor * dst, const lhm_ubatch * ubatch) const;
