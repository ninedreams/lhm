...
lhm_kv_cache::lhm_kv_cache(
    ...
    other(static_cast<lhm_kv_cache *>(mem_other)),
    v_cells_impl(other ? other->v_cells_impl : std::make_shared<lhm_kv_cells_vec>()),
    ...
    seq_to_stream.resize(LHM_MAX_SEQ, 0);
    ...
    if (other) {
        ...
        if ((model.arch == LLM_ARCH_DEEPSEEK4) && (hparams.n_embd_head_k_full == hparams.indexer_head_size)) {
            attn_rot_k = true;
        }
    ...
    const char * LHM_KV_CACHE_DEBUG = getenv("LHM_KV_CACHE_DEBUG");
    debug = LHM_KV_CACHE_DEBUG ? atoi(LHM_KV_CACHE_DEBUG) : 0;
}

void lhm_kv_cache::clear(bool data) {
    for (uint32_t s = 0; s < n_stream; ++s) {
        v_cells[s].reset();
        v_heads[s] = 0;
    }
...
bool lhm_kv_cache::seq_rm(lhm_seq_id seq_id, lhm_pos p0, lhm_pos p1) {
    // TODO: refactor [TAG_KV_CACHE_SHARE_CELLS]
    if (other) {
        return true;
    }
...
void lhm_kv_cache::seq_cp(lhm_seq_id seq_id_src, lhm_seq_id seq_id_dst, lhm_pos p0, lhm_pos p1) {
    // TODO: refactor [TAG_KV_CACHE_SHARE_CELLS]
    if (other) {
        return;
    }
    ...
    for (uint32_t i = 0; i < v_cells[s0].size(); ++i) {
        if (v_cells[s0].seq_has(i, seq_id_src)) {
            lhm_pos pos   = v_cells[s0].pos_get(i);
            lhm_pos shift = v_cells[s0].get_shift(i);

            lhm_kv_cell_ext ext = v_cells[s0].ext_get(i);
...
void lhm_kv_cache::seq_add(lhm_seq_id seq_id, lhm_pos p0, lhm_pos p1, lhm_pos shift) {
    // TODO: refactor [TAG_KV_CACHE_SHARE_CELLS]
    if (other) {
        return;
    }
...
void lhm_kv_cache::seq_div(lhm_seq_id seq_id, lhm_pos p0, lhm_pos p1, int d) {
    // TODO: refactor [TAG_KV_CACHE_SHARE_CELLS]
    if (other) {
        return;
    }
...
lhm_pos lhm_kv_cache::seq_pos_min(lhm_seq_id seq_id) const {
    // TODO: refactor [TAG_KV_CACHE_SHARE_CELLS]
    if (other) {
...
lhm_pos lhm_kv_cache::seq_pos_max(lhm_seq_id seq_id) const {
    // TODO: refactor [TAG_KV_CACHE_SHARE_CELLS]
    if (other) {
...
std::map<ggml_backend_buffer_type_t, size_t> lhm_kv_cache::memory_breakdown() const {
...
lhm_memory_context_ptr lhm_kv_cache::init_batch(
    ...
    do {
        ...
        return std::make_unique<lhm_kv_cache_context>(
                this, std::move(sinfos), std::move(ubatches));
    } while (false);

    return std::make_unique<lhm_kv_cache_context>(LHM_MEMORY_STATUS_FAILED_PREPARE);
}

lhm_memory_context_ptr lhm_kv_cache::init_full() {
    return std::make_unique<lhm_kv_cache_context>(this);
}

lhm_memory_context_ptr lhm_kv_cache::init_update(lhm_context * lctx, bool optimize) {
    GGML_UNUSED(optimize);

    bool do_shift = get_has_shift();

    return std::make_unique<lhm_kv_cache_context>(this, lctx, do_shift, std::move(sc_info));
}

lhm_kv_cache::slot_info_vec_t lhm_kv_cache::prepare(const std::vector<lhm_ubatch> & ubatches) {
    lhm_kv_cache::slot_info_vec_t res;
    ...
    struct state_t {
        ...
        std::vector<lhm_kv_cells> v_cells; // copy of the old cells, before placing the ubatch
    };
...
bool lhm_kv_cache::update(lhm_context * lctx, bool do_shift, const stream_copy_info & sc_info) {
    // TODO: refactor [TAG_KV_CACHE_SHARE_CELLS]
    if (other) {
        return true;
    }
...
lhm_kv_cache::slot_info lhm_kv_cache::find_slot(const lhm_ubatch & ubatch, bool cont) const {
        ...
        while (true) {
            ...
            for (uint32_t i = 0; i < n_test; i++) {
                ...
                if (!can_use && cells.seq_count(idx) == 1) {
                    const lhm_pos pos_cell = cells.pos_get(idx);
                    ...
                    if (!can_use) {
                        const lhm_seq_id seq_id_cell = cells.seq_get(idx);

                        // SWA mask
                        if (lhm_hparams::is_masked_swa(n_swa, swa_type, pos_cell, cells.seq_pos_max(seq_id_cell) + 1)) {
                            can_use = true;
                        }
                    }
                }
...
void lhm_kv_cache::apply_ubatch(const slot_info & sinfo, const lhm_ubatch & ubatch) {
    // TODO: refactor [TAG_KV_CACHE_SHARE_CELLS]
    if (other) {
        return;
    }
    ...
    lhm_seq_id seq_pos_max_rm[LHM_MAX_SEQ];
    ...
    for (uint32_t s = 0; s < sinfo.n_stream(); ++s) {
        for (uint32_t ii = 0; ii < sinfo.size(); ++ii) {
            ...
            if (!cells.is_empty(idx)) {
                ...
                const lhm_seq_id seq_id = cells.seq_get(idx);
                const lhm_pos    pos    = cells.pos_get(idx);
...
bool lhm_kv_cache::get_can_shift() const {
...
uint32_t lhm_kv_cache::get_size() const {
    const auto & cells = v_cells[seq_to_stream[0]];

    return cells.size();
}

uint32_t lhm_kv_cache::get_n_stream() const {
    return n_stream;
}

bool lhm_kv_cache::get_has_shift() const {
    bool result = false;
...
ggml_type lhm_kv_cache::type_k() const {
    return layers[0].k->type;
}

ggml_type lhm_kv_cache::type_v() const {
    return layers[0].v->type;
}

std::vector<uint32_t> lhm_kv_cache::get_layer_ids() const {
...
ggml_tensor * lhm_kv_cache::get_k_storage(int32_t il) const {
    const int32_t ikv = map_layer_ids.at(il);

    return layers[ikv].k;
}

uint32_t lhm_kv_cache::get_n_kv(const slot_info & sinfo) const {
    uint32_t result = 0;
...
ggml_tensor * lhm_kv_cache::get_k(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    const int32_t ikv = map_layer_ids.at(il);
...
ggml_tensor * lhm_kv_cache::get_v(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    const int32_t ikv = map_layer_ids.at(il);
...
ggml_tensor * lhm_kv_cache::cpy_k(ggml_context * ctx, ggml_tensor * k_cur, ggml_tensor * k_idxs, int32_t il, const slot_info & sinfo) const {
    GGML_UNUSED(sinfo);
...
ggml_tensor * lhm_kv_cache::cpy_v(ggml_context * ctx, ggml_tensor * v_cur, ggml_tensor * v_idxs, int32_t il, const slot_info & sinfo) const {
    GGML_UNUSED(sinfo);
...
ggml_tensor * lhm_kv_cache::build_input_k_idxs(ggml_context * ctx, const lhm_ubatch & ubatch) const {
...
ggml_tensor * lhm_kv_cache::build_input_v_idxs(ggml_context * ctx, const lhm_ubatch & ubatch) const {
    const uint32_t n_tokens = ubatch.n_tokens;
...
ggml_tensor * lhm_kv_cache::build_input_k_rot(ggml_context * ctx) const {
...
ggml_tensor * lhm_kv_cache::build_input_v_rot(ggml_context * ctx) const {
    ggml_tensor * res = nullptr;
...
struct args_set_input_kq_mask {
    ...
    const std::vector<lhm_kv_cells> & v_cells;
...
static void set_input_kq_mask_impl(const args_set_input_kq_mask & args, T * data) {
  //const auto & hparams = args.hparams;
    ...
    const lhm_swa_type swa_type = args.swa_type;

    const int64_t n_kv     = args.n_kv;
    const int64_t n_stream = args.n_stream;
    const int64_t n_tps    = args.n_tps;

    const T mask_keep = lhm_cast<T>(0.0f);
    const T mask_drop = lhm_cast<T>(-INFINITY);

    // the min position in the batch for each sequence
    lhm_pos seq_pos_min[LHM_MAX_SEQ];
    std::fill(seq_pos_min, seq_pos_min + LHM_MAX_SEQ, INT32_MAX);
    ...
    for (uint32_t s = 0; s < n_stream; ++s) {
        ...
        std::unordered_map<lhm_seq_id, uint32_t>              seq_srct;
        std::unordered_map<lhm_seq_id, std::vector<uint32_t>> seq_idxs;
        ...
        for (uint32_t ii = 0; ii < n_tps; ++ii) {
            ...
            const auto & cells = v_cells.at(seq_to_stream[seq_id]);
                  ...
                  lhm_pos p0 = -1;
            const lhm_pos p1 = ubatch->pos[i];

            // for M-RoPE
            const lhm_pos p1_x = is_2d ? ubatch->pos[i + ubatch->n_tokens*2] : 0;
            const lhm_pos p1_y = is_2d ? ubatch->pos[i + ubatch->n_tokens]   : 0;
            ...
            for (uint32_t jj = 0; jj < n_kv; ++jj) {
                ...
                // apply SWA if any
                if (swa) {
                    if (lhm_hparams::is_masked_swa(n_swa, swa_type, p0, p1)) {
                        goto skip;
                    }
                }

                if (alibi) {
                    data[idst + j] = lhm_cast<T>(static_cast<float>(-std::abs(p0 - p1)));
                } else {
...
static void set_input_kq_mask_impl(const args_set_input_kq_mask & args, T * data) {
    const bool swa = args.swa_type != LHM_SWA_TYPE_NONE;
    if (swa) {
    ...
    for (int h = 0; h < 1; ++h) {
        for (int i = 0; i < n_tokens; ++i) {
            for (int j = 0; j < n_kv; ++j) {
                ...
                const lhm_pos p0 = cells.is_empty(j) ? -1 : cells.pos_get(j);

                data[h*(n_kv*n_tokens) + i*n_kv + j] = lhm_relative_position_bucket(p0, ubatch->pos[i], hparams.n_rel_attn_bkts, false);
            }
        }
    }
}
...
size_t lhm_kv_cache::total_size() const {
    size_t size = 0;
...
size_t lhm_kv_cache::size_k_bytes() const {
...
size_t lhm_kv_cache::size_v_bytes() const {
...
ggml_tensor * lhm_kv_cache::build_rope_shift(
    ...
    const auto & rope_type = hparams.rope_type == LHM_ROPE_TYPE_MROPE || hparams.rope_type == LHM_ROPE_TYPE_IMROPE
...
public:
    llm_graph_input_k_shift(const lhm_kv_cache * kv_self) : kv_self(kv_self) {}
    ...
    const lhm_kv_cache * kv_self;
};

void llm_graph_input_k_shift::set_input(const lhm_ubatch * ubatch) {
    GGML_UNUSED(ubatch);
...
void lhm_kv_cache::state_write(lhm_io_write_i & io, lhm_seq_id seq_id, lhm_state_seq_flags flags) const {
    // TODO: refactor [TAG_KV_CACHE_SHARE_CELLS]
    if (other) {
        return;
    }
    ...
    for (uint32_t s = 0; s < n_stream; ++s) {
        ...
        for (uint32_t i = 0; i < cells.size(); ++i) {
            ...
            if (add_cell && seq_id != -1) {
                const bool is_masked = lhm_hparams::is_masked_swa(n_swa, swa_type, cells.pos_get(i), cells.seq_pos_max(seq_id));
...
void lhm_kv_cache::state_read(lhm_io_read_i & io, lhm_seq_id seq_id, lhm_state_seq_flags flags) {
    // TODO: refactor [TAG_KV_CACHE_SHARE_CELLS]
    if (other) {
        return;
    }
...
void lhm_kv_cache::state_write_meta(lhm_io_write_i & io, const cell_ranges_t & cr, lhm_seq_id seq_id) const {
    const auto & cells = v_cells[cr.strm];

    for (const auto & range : cr.data) {
        for (uint32_t i = range.first; i < range.second; ++i) {
            std::vector<lhm_seq_id> seq_ids;

            for (lhm_seq_id cur = 0; cur < (int) n_seq_max; ++cur) {
            ...
            const lhm_pos pos     = cells.pos_get(i);
            ...
            if (hparams.n_pos_per_embd() > 1) {
                const lhm_kv_cell_ext ext = cells.ext_get(i);
...
void lhm_kv_cache::state_write_data(lhm_io_write_i & io, const cell_ranges_t & cr) const {
    const auto & cells = v_cells[cr.strm];
...
bool lhm_kv_cache::state_read_meta(lhm_io_read_i & io, uint32_t strm, uint32_t cell_count, slot_info & sinfo, lhm_seq_id dest_seq_id) {
...
bool lhm_kv_cache::state_read_data(lhm_io_read_i & io, uint32_t strm, uint32_t cell_count, const slot_info & sinfo) {
    auto & cells = v_cells[strm];
...
lhm_kv_cache_context::lhm_kv_cache_context(lhm_memory_status status) : status(status) {}

lhm_kv_cache_context::lhm_kv_cache_context(
        lhm_kv_cache * kv) : status(LHM_MEMORY_STATUS_SUCCESS), kv(kv) {
    n_kv = kv->get_size();
...
lhm_kv_cache_context::lhm_kv_cache_context(
        ...
        lhm_context * lctx,
        bool do_shift,
        stream_copy_info sc_info) : status(LHM_MEMORY_STATUS_SUCCESS), kv(kv), lctx(lctx), do_shift(do_shift), sc_info(std::move(sc_info)) {
    if (!do_shift && this->sc_info.empty()) {
        status = LHM_MEMORY_STATUS_NO_UPDATE;
    }
}
...
lhm_kv_cache_context::lhm_kv_cache_context(
        ...
        std::vector<lhm_ubatch> ubatches) : status(LHM_MEMORY_STATUS_SUCCESS), kv(kv), sinfos(std::move(sinfos)), ubatches(std::move(ubatches)) {
}

lhm_kv_cache_context::~lhm_kv_cache_context() = default;

bool lhm_kv_cache_context::next() {
...
bool lhm_kv_cache_context::apply() {
...
lhm_memory_status lhm_kv_cache_context::get_status() const {
    return status;
}

const lhm_ubatch & lhm_kv_cache_context::get_ubatch() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);

    return ubatches[i_cur];
}

uint32_t lhm_kv_cache_context::get_n_kv() const {
    return n_kv;
}

ggml_type lhm_kv_cache_context::type_k() const {
    return kv->type_k();
}

ggml_type lhm_kv_cache_context::type_v() const {
    return kv->type_v();
}

ggml_tensor * lhm_kv_cache_context::get_k(ggml_context * ctx, int32_t il) const {
    return kv->get_k(ctx, il, n_kv, sinfos[i_cur]);
}

ggml_tensor * lhm_kv_cache_context::get_v(ggml_context * ctx, int32_t il) const {
    return kv->get_v(ctx, il, n_kv, sinfos[i_cur]);
}

ggml_tensor * lhm_kv_cache_context::cpy_k(ggml_context * ctx, ggml_tensor * k_cur, ggml_tensor * k_idxs, int32_t il) const {
    return kv->cpy_k(ctx, k_cur, k_idxs, il, sinfos[i_cur]);
}

ggml_tensor * lhm_kv_cache_context::cpy_v(ggml_context * ctx, ggml_tensor * v_cur, ggml_tensor * v_idxs, int32_t il) const {
    return kv->cpy_v(ctx, v_cur, v_idxs, il, sinfos[i_cur]);
}

ggml_tensor * lhm_kv_cache_context::build_input_k_idxs(ggml_context * ctx, const lhm_ubatch & ubatch) const {
    return kv->build_input_k_idxs(ctx, ubatch);
}

ggml_tensor * lhm_kv_cache_context::build_input_v_idxs(ggml_context * ctx, const lhm_ubatch & ubatch) const {
    return kv->build_input_v_idxs(ctx, ubatch);
}

ggml_tensor * lhm_kv_cache_context::build_input_k_rot(ggml_context * ctx) const {
    return kv->build_input_k_rot(ctx);
}

ggml_tensor * lhm_kv_cache_context::build_input_v_rot(ggml_context * ctx) const {
    return kv->build_input_v_rot(ctx);
}

void lhm_kv_cache_context::set_input_k_shift(ggml_tensor * dst) const {
    kv->set_input_k_shift(dst);
}

void lhm_kv_cache_context::set_input_k_idxs(ggml_tensor * dst, const lhm_ubatch * ubatch) const {
    kv->set_input_k_idxs(dst, ubatch, sinfos[i_cur]);
}

void lhm_kv_cache_context::set_input_v_idxs(ggml_tensor * dst, const lhm_ubatch * ubatch) const {
    kv->set_input_v_idxs(dst, ubatch, sinfos[i_cur]);
}

void lhm_kv_cache_context::set_input_kq_mask(ggml_tensor * dst, const lhm_ubatch * ubatch, bool causal_attn) const {
    kv->set_input_kq_mask(dst, ubatch, causal_attn);
}

void lhm_kv_cache_context::set_input_pos_bucket(ggml_tensor * dst, const lhm_ubatch * ubatch) const {
    kv->set_input_pos_bucket(dst, ubatch);
}

void lhm_kv_cache_context::set_input_k_rot(ggml_tensor * dst) const {
    kv->set_input_k_rot(dst);
}

void lhm_kv_cache_context::set_input_v_rot(ggml_tensor * dst) const {
