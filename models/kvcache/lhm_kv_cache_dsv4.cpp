...
static int64_t dsv4_stream_offset(uint32_t n_stream, lhm_seq_id seq_id, uint32_t size) {
    if (n_stream <= 1) {
        return 0;
    }
...
static bool dsv4_ubatch_has_coupled(const lhm_ubatch & ubatch) {
...
static bool dsv4_token_has_seq(const lhm_ubatch & ubatch, uint32_t i, lhm_seq_id seq_id) {
...
static lhm_ubatch dsv4_build_raw_write_ubatch(const lhm_ubatch & ubatch) {
    ...
    auto data = std::make_shared<lhm_ubatch::data_t>();
    ...
    data->seq_idx.assign(LHM_MAX_SEQ, -1);
...
static std::vector<lhm_ubatch> dsv4_build_raw_write_ubatches(const std::vector<lhm_ubatch> & ubatches) {
    std::vector<lhm_ubatch> res;
    res.reserve(ubatches.size());
    for (const lhm_ubatch & ubatch : ubatches) {
        res.push_back(dsv4_build_raw_write_ubatch(ubatch));
    }
    return res;
}

static bool dsv4_batch_has_coupled(const lhm_batch & batch) {
    if (!batch.n_seq_id) {
        return false;
    }
...
static int64_t dsv4_comp_graph_n_stream(const lhm_ubatch & ubatch, uint32_t n_stream) {
...
static lhm_kv_cache_dsv4_context::comp_plan dsv4_build_comp_plan(
        ...
        uint32_t kv_size,
        uint32_t n_stream) {
    lhm_kv_cache_dsv4_context::comp_plan plan;
    ...
    std::map<std::pair<lhm_seq_id, lhm_pos>, int64_t> curr_token_idx_map;
    ...
    const auto state_source_idx = [&](lhm_seq_id seq_id, lhm_pos pos) -> int32_t {
        if (pos < 0) {
    ...
    for (uint32_t i = 0; i < ubatch.n_tokens; ++i) {
        const lhm_pos pos = ubatch.pos[i];
        ...
        for (int32_t s = 0; s < ubatch.n_seq_id[i]; ++s) {
            const lhm_seq_id seq_id = ubatch.seq_id[i][s];
            const int64_t stream_off = dsv4_stream_offset(n_stream, seq_id, state_size);
            ...
            const lhm_pos source_start = pos + 1 - ratio;
            ...
            if (overlap) {
                const lhm_pos prev_start = source_start - ratio;
...
static std::vector<lhm_kv_cache_dsv4_context::comp_plan> dsv4_build_comp_plans(
        const std::vector<lhm_ubatch> & ubatches,
        uint32_t ratio,
        bool overlap,
        uint32_t state_size,
        uint32_t kv_size,
        uint32_t n_stream) {
    std::vector<lhm_kv_cache_dsv4_context::comp_plan> plans;
...
static lhm_kv_cache::slot_info_vec_t dsv4_build_comp_sinfos(
        const std::vector<lhm_ubatch> & ubatches,
        uint32_t n_stream) {
    lhm_kv_cache::slot_info_vec_t sinfos;
    ...
    for (const lhm_ubatch & ubatch : ubatches) {
        ...
        lhm_kv_cache::slot_info sinfo;
        sinfo.s0 = n_stream > 1 ? LHM_MAX_SEQ : 0;
        sinfo.s1 = 0;
        sinfo.resize(ns);

        for (uint32_t s = 0; s < ns; ++s) {
            const lhm_seq_id seq_id = n_stream > 1 ? ubatch.seq_id_unq[s] : 0;
...
static lhm_kv_cache::slot_info_vec_t dsv4_build_raw_read_sinfos(
        const lhm_kv_cache::slot_info_vec_t & sinfos_write,
        const std::vector<lhm_ubatch> & ubatches) {
    lhm_kv_cache::slot_info_vec_t sinfos;
    sinfos.reserve(ubatches.size());

    for (size_t i = 0; i < ubatches.size(); ++i) {
        const lhm_ubatch & ubatch = ubatches[i];
        ...
        const lhm_seq_id seq_id = ubatch.seq_id[0][0];
        uint32_t i_stream = 0;
...
static lhm_kv_cache_dsv4_context::comp_plan dsv4_build_reserve_comp_plan(
        const lhm_ubatch & ubatch,
        uint32_t ratio,
        bool overlap,
        uint32_t state_size,
...
static void dsv4_make_k_only(lhm_hparams & hparams) {
    // lhm_kv_cache uses hparams.is_mla() to allocate K-only storage.
...
lhm_dsv4_comp_state::lhm_dsv4_comp_state(
    ...
    const lhm_hparams & hparams = model.hparams;
...
uint32_t lhm_dsv4_comp_state::get_ratio() const {
    return ratio;
}

uint32_t lhm_dsv4_comp_state::get_state_size() const {
    return state_size;
}

uint32_t lhm_dsv4_comp_state::get_n_stream() const {
    return n_stream;
}

std::map<ggml_backend_buffer_type_t, size_t> lhm_dsv4_comp_state::memory_breakdown() const {
...
void lhm_dsv4_comp_state::state_write(lhm_io_write_i & io, lhm_seq_id seq_id, lhm_state_seq_flags flags) const {
    GGML_UNUSED(flags);
...
void lhm_dsv4_comp_state::state_read(lhm_io_read_i & io, lhm_seq_id seq_id, lhm_state_seq_flags flags) {
    GGML_UNUSED(flags);
...
ggml_tensor * lhm_dsv4_comp_state::get_kv(ggml_context * ctx, int32_t il) const {
...
ggml_tensor * lhm_dsv4_comp_state::get_score(ggml_context * ctx, int32_t il) const {
    const int32_t ids = map_layer_ids.at(il);
...
ggml_tensor * lhm_dsv4_comp_state::cpy_kv(ggml_context * ctx, ggml_tensor * cur, ggml_tensor * idxs, int32_t il) const {
    return ggml_set_rows(ctx, get_kv(ctx, il), cur, idxs);
}

ggml_tensor * lhm_dsv4_comp_state::cpy_score(ggml_context * ctx, ggml_tensor * cur, ggml_tensor * idxs, int32_t il) const {
    return ggml_set_rows(ctx, get_score(ctx, il), cur, idxs);
}

size_t lhm_dsv4_comp_state::total_size() const {
    size_t size = 0;
...
lhm_kv_cache_dsv4::lhm_kv_cache_dsv4(
        const lhm_model & model,
                ggml_type   type_k,
                ggml_type   type_v,
                     bool   v_trans,
    ...
    kv_raw = std::make_unique<lhm_kv_cache_iswa>(
...
lhm_memory_context_ptr lhm_kv_cache_dsv4::init_batch(
    ...
    const auto make_context = [&](std::vector<lhm_ubatch> ubatches) -> lhm_memory_context_ptr {
        ...
        return std::make_unique<lhm_kv_cache_dsv4_context>(
                this,
    ...
    // Match lhm_kv_cache_iswa splitting when DSV4 compressed state does not
    ...
    return std::make_unique<lhm_kv_cache_dsv4_context>(LHM_MEMORY_STATUS_FAILED_PREPARE);
}

lhm_memory_context_ptr lhm_kv_cache_dsv4::init_full() {
    return std::make_unique<lhm_kv_cache_dsv4_context>(this);
}

lhm_memory_context_ptr lhm_kv_cache_dsv4::init_update(lhm_context * lctx, bool optimize) {
    return std::make_unique<lhm_kv_cache_dsv4_context>(this, lctx, optimize);
}

bool lhm_kv_cache_dsv4::get_can_shift() const {
...
void lhm_kv_cache_dsv4::clear(bool data) {
    kv_raw->clear(data);
    clear_compressed(true); // DSV4 compressed buffers must never expose stale/uninit rows
}

bool lhm_kv_cache_dsv4::seq_rm(lhm_seq_id seq_id, lhm_pos p0, lhm_pos p1) {
    if (p1 >= 0) {
        return false;
    }
...
void lhm_kv_cache_dsv4::seq_cp(lhm_seq_id seq_id_src, lhm_seq_id seq_id_dst, lhm_pos p0, lhm_pos p1) {
    kv_raw->seq_cp(seq_id_src, seq_id_dst, p0, p1);
    clear_compressed(true);
}

void lhm_kv_cache_dsv4::seq_keep(lhm_seq_id seq_id) {
    kv_raw->seq_keep(seq_id);
    clear_compressed(true);
}

void lhm_kv_cache_dsv4::seq_add(lhm_seq_id seq_id, lhm_pos p0, lhm_pos p1, lhm_pos shift) {
    kv_raw->seq_add(seq_id, p0, p1, shift);
    clear_compressed(true);
}

void lhm_kv_cache_dsv4::seq_div(lhm_seq_id seq_id, lhm_pos p0, lhm_pos p1, int d) {
    kv_raw->seq_div(seq_id, p0, p1, d);
    clear_compressed(true);
}

lhm_pos lhm_kv_cache_dsv4::seq_pos_min(lhm_seq_id seq_id) const {
...
lhm_pos lhm_kv_cache_dsv4::seq_pos_max(lhm_seq_id seq_id) const {
    if (seq_id < 0 || (uint32_t) seq_id >= n_seq_max) {
        return -1;
    }
...
std::map<ggml_backend_buffer_type_t, size_t> lhm_kv_cache_dsv4::memory_breakdown() const {
...
void lhm_kv_cache_dsv4::state_write(lhm_io_write_i & io, lhm_seq_id seq_id, lhm_state_seq_flags flags) const {
    const bool partial_only = flags & LHM_STATE_SEQ_FLAGS_PARTIAL_ONLY;
...
void lhm_kv_cache_dsv4::state_read(lhm_io_read_i & io, lhm_seq_id seq_id, lhm_state_seq_flags flags) {
    uint32_t magic;
    uint32_t version;
    ...
    if (partial_only != !!(flags & LHM_STATE_SEQ_FLAGS_PARTIAL_ONLY)) {
...
lhm_kv_cache_iswa * lhm_kv_cache_dsv4::get_raw() const {
    return kv_raw.get();
}

lhm_kv_cache * lhm_kv_cache_dsv4::get_csa() const {
    return kv_csa.get();
}

lhm_kv_cache * lhm_kv_cache_dsv4::get_hca() const {
    return kv_hca.get();
}

lhm_kv_cache * lhm_kv_cache_dsv4::get_lid() const {
    return kv_lid.get();
}

lhm_dsv4_comp_state * lhm_kv_cache_dsv4::get_csa_state() const {
    return csa_state.get();
}

lhm_dsv4_comp_state * lhm_kv_cache_dsv4::get_hca_state() const {
    return hca_state.get();
}

lhm_dsv4_comp_state * lhm_kv_cache_dsv4::get_lid_state() const {
    return lid_state.get();
}

void lhm_kv_cache_dsv4::clear_compressed(bool data) {
    kv_csa->clear(data);
    kv_hca->clear(data);
    kv_lid->clear(data);
...
static lhm_kv_cache::slot_info dsv4_build_full_sinfo(const lhm_kv_cache * kv) {
...
lhm_kv_cache_dsv4_raw_context::lhm_kv_cache_dsv4_raw_context(lhm_kv_cache_iswa * kv) :
    kv_swa(kv->get_swa()),
    ctx_base_mem(nullptr),
    ctx_swa_mem(nullptr),
    n_kv(kv_swa->get_size()),
    status(LHM_MEMORY_STATUS_SUCCESS) {
    sinfos_read.push_back(dsv4_build_full_sinfo(kv_swa));
    sinfos_write = sinfos_read;
}

lhm_kv_cache_dsv4_raw_context::lhm_kv_cache_dsv4_raw_context(
    ...
    n_kv(kv_swa->get_size()),
    status(lhm_memory_status_combine(ctx_base_mem->get_status(), ctx_swa_mem->get_status())) {
}
...
lhm_kv_cache_dsv4_raw_context::lhm_kv_cache_dsv4_raw_context(
    ...
    ctx_base_mem(std::make_unique<lhm_kv_cache_context>(
...
bool lhm_kv_cache_dsv4_raw_context::next() {
    if (ubatches.empty()) {
        return true;
    }
...
bool lhm_kv_cache_dsv4_raw_context::apply() {
    bool res = true;
...
lhm_memory_status lhm_kv_cache_dsv4_raw_context::get_status() const {
    return status;
}

const lhm_ubatch & lhm_kv_cache_dsv4_raw_context::get_ubatch() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);

    return ubatches[i_next];
}

uint32_t lhm_kv_cache_dsv4_raw_context::get_n_kv() const {
    return n_kv;
}

uint32_t lhm_kv_cache_dsv4_raw_context::get_n_write() const {
    if (ubatches_write.empty()) {
        return 0;
    }
...
ggml_tensor * lhm_kv_cache_dsv4_raw_context::get_k(ggml_context * ctx, int32_t il) const {
    return kv_swa->get_k(ctx, il, n_kv, sinfos_read[i_next]);
}

ggml_tensor * lhm_kv_cache_dsv4_raw_context::cpy_k(ggml_context * ctx, ggml_tensor * k_cur, ggml_tensor * k_idxs, int32_t il) const {
...
ggml_tensor * lhm_kv_cache_dsv4_raw_context::build_input_k_idxs(ggml_context * ctx, const lhm_ubatch & ubatch) const {
...
ggml_tensor * lhm_kv_cache_dsv4_raw_context::build_input_k_rot(ggml_context * ctx) const {
    return kv_swa->build_input_k_rot(ctx);
}

void lhm_kv_cache_dsv4_raw_context::set_input_k_idxs(ggml_tensor * dst) const {
    kv_swa->set_input_k_idxs(dst, &ubatches_write[i_next], sinfos_write[i_next]);
}

void lhm_kv_cache_dsv4_raw_context::set_input_kq_mask(ggml_tensor * dst, const lhm_ubatch * ubatch, bool causal_attn) const {
    kv_swa->set_input_kq_mask(dst, ubatch, causal_attn);
}

void lhm_kv_cache_dsv4_raw_context::set_input_k_rot(ggml_tensor * dst) const {
...
lhm_kv_cache_dsv4_comp_context::lhm_kv_cache_dsv4_comp_context(lhm_kv_cache * kv) : kv(kv), n_kv(kv->get_size()) {
    const uint32_t n_stream = kv->get_n_stream();
...
lhm_kv_cache_dsv4_comp_context::lhm_kv_cache_dsv4_comp_context(
        lhm_kv_cache * kv,
...
bool lhm_kv_cache_dsv4_comp_context::next() {
    if (ubatches.empty()) {
        return true;
    }
...
uint32_t lhm_kv_cache_dsv4_comp_context::get_n_kv() const {
    return n_kv;
}

ggml_tensor * lhm_kv_cache_dsv4_comp_context::get_k(ggml_context * ctx, int32_t il) const {
    return kv->get_k(ctx, il, n_kv, sinfos[i_cur]);
}

ggml_tensor * lhm_kv_cache_dsv4_comp_context::cpy_k(ggml_context * ctx, ggml_tensor * k_cur, ggml_tensor * k_idxs, int32_t il) const {
    return kv->cpy_k(ctx, k_cur, k_idxs, il, sinfos[i_cur]);
}

ggml_tensor * lhm_kv_cache_dsv4_comp_context::build_input_k_rot(ggml_context * ctx) const {
    return kv->build_input_k_rot(ctx);
}

void lhm_kv_cache_dsv4_comp_context::set_input_k_rot(ggml_tensor * dst) const {
...
lhm_kv_cache_dsv4_context::lhm_kv_cache_dsv4_context(lhm_memory_status status) : status(status) {}

lhm_kv_cache_dsv4_context::lhm_kv_cache_dsv4_context(
        lhm_kv_cache_dsv4 * kv) :
    ctx_raw(std::make_unique<lhm_kv_cache_dsv4_raw_context>(kv->get_raw())),
    ctx_csa_mem(kv->get_csa()->init_full()),
    ctx_hca_mem(kv->get_hca()->init_full()),
    ctx_lid_mem(kv->get_lid()->init_full()),
    ctx_csa(std::make_unique<lhm_kv_cache_dsv4_comp_context>(kv->get_csa())),
    ctx_hca(std::make_unique<lhm_kv_cache_dsv4_comp_context>(kv->get_hca())),
    ctx_lid(std::make_unique<lhm_kv_cache_dsv4_comp_context>(kv->get_lid())),
    csa_state(kv->get_csa_state()),
    hca_state(kv->get_hca_state()),
    lid_state(kv->get_lid_state()),
    reserve_plans(true),
    status(lhm_memory_status_combine(
                lhm_memory_status_combine(ctx_raw->get_status(), ctx_csa_mem->get_status()),
                lhm_memory_status_combine(ctx_hca_mem->get_status(), ctx_lid_mem->get_status()))) {
}
...
lhm_kv_cache_dsv4_context::lhm_kv_cache_dsv4_context(
        ...
        lhm_context * lctx,
        bool optimize) :
    ctx_raw(std::make_unique<lhm_kv_cache_dsv4_raw_context>(kv->get_raw(), lctx, optimize)),
...
lhm_kv_cache_dsv4_context::lhm_kv_cache_dsv4_context(
    ...
    plans_lid(plans_csa),
    ctx_raw(std::make_unique<lhm_kv_cache_dsv4_raw_context>(
                kv->get_raw(),
                std::move(sinfos_raw_base_write),
                std::move(sinfos_raw_swa_write),
                std::move(sinfos_raw_swa_read),
    ...
    ctx_hca_mem(nullptr),
    ctx_lid_mem(nullptr),
    ctx_csa(std::make_unique<lhm_kv_cache_dsv4_comp_context>(
                kv->get_csa(),
                dsv4_build_comp_sinfos(this->ubatches, kv->get_csa()->get_n_stream()),
                this->ubatches)),
    ctx_hca(std::make_unique<lhm_kv_cache_dsv4_comp_context>(
                kv->get_hca(),
                dsv4_build_comp_sinfos(this->ubatches, kv->get_hca()->get_n_stream()),
                this->ubatches)),
    ctx_lid(std::make_unique<lhm_kv_cache_dsv4_comp_context>(
                kv->get_lid(),
...
lhm_kv_cache_dsv4_context::~lhm_kv_cache_dsv4_context() = default;

bool lhm_kv_cache_dsv4_context::next() {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);
...
bool lhm_kv_cache_dsv4_context::apply() {
...
lhm_memory_status lhm_kv_cache_dsv4_context::get_status() const {
    return status;
}

const lhm_ubatch & lhm_kv_cache_dsv4_context::get_ubatch() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);

    return ubatches[i_next];
}

const lhm_kv_cache_dsv4_raw_context * lhm_kv_cache_dsv4_context::get_raw() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);

    return ctx_raw.get();
}

const lhm_kv_cache_dsv4_comp_context * lhm_kv_cache_dsv4_context::get_csa() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);

    return ctx_csa.get();
}

const lhm_kv_cache_dsv4_comp_context * lhm_kv_cache_dsv4_context::get_hca() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);

    return ctx_hca.get();
}

const lhm_kv_cache_dsv4_comp_context * lhm_kv_cache_dsv4_context::get_lid() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);

    return ctx_lid.get();
}

const lhm_dsv4_comp_state * lhm_kv_cache_dsv4_context::get_csa_state() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);

    return csa_state;
}

const lhm_dsv4_comp_state * lhm_kv_cache_dsv4_context::get_hca_state() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);

    return hca_state;
}

const lhm_dsv4_comp_state * lhm_kv_cache_dsv4_context::get_lid_state() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);

    return lid_state;
}

const lhm_kv_cache_dsv4_context::comp_plan & lhm_kv_cache_dsv4_context::get_csa_plan() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);
...
const lhm_kv_cache_dsv4_context::comp_plan & lhm_kv_cache_dsv4_context::get_hca_plan() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);
...
const lhm_kv_cache_dsv4_context::comp_plan & lhm_kv_cache_dsv4_context::get_lid_plan() const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);
...
const lhm_kv_cache_dsv4_context::comp_plan & lhm_kv_cache_dsv4_context::get_csa_plan(const lhm_ubatch & ubatch) const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);
...
const lhm_kv_cache_dsv4_context::comp_plan & lhm_kv_cache_dsv4_context::get_hca_plan(const lhm_ubatch & ubatch) const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);
...
const lhm_kv_cache_dsv4_context::comp_plan & lhm_kv_cache_dsv4_context::get_lid_plan(const lhm_ubatch & ubatch) const {
    assert(status == LHM_MEMORY_STATUS_SUCCESS);
