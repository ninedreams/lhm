...
struct lhm_layer {
    ...
    struct lhm_layer_posnet posnet;

    struct lhm_layer_convnext convnext;

    struct lhm_layer_shortconv shortconv;

    struct lhm_layer_nextn nextn;
};
...
struct lhm_meta_device_get_split_state_userdata {
    size_t                     n_devices;
...
struct lhm_model {
    ...
    std::vector<lhm_device> devices;

    // for quantize-stats only
    std::vector<std::pair<std::string, struct ggml_tensor *>> tensors_by_name;

    // for keeping track of associated LoRA adapters
    std::unordered_set<lhm_adapter_lora *> loras;

    // statically allocated context for assigning
    struct lhm_meta_device_get_split_state_userdata get_split_state_ud;

    int64_t t_load_us  = 0;
    int64_t t_start_us = 0;

    explicit lhm_model(const lhm_model_params & params);
    virtual ~lhm_model();
    ...
    lhm_split_mode split_mode() const;
    ...
    float get_rope_freq_base (const lhm_cparams & cparams, int il) const;
    float get_rope_freq_scale(const lhm_cparams & cparams, int il) const;

    ggml_tensor * get_rope_factors(const lhm_cparams & cparams, int il) const;

    lhm_memory_i * create_memory(const lhm_memory_params & params, const lhm_cparams & cparams) const;

    ggml_cgraph * build_graph(const llm_graph_params & params) const;

    virtual void load_stats  (lhm_model_loader & ml) = 0;
    virtual void load_hparams(lhm_model_loader & ml) = 0;
    virtual void load_vocab  (lhm_model_loader & ml) = 0;
    virtual bool load_tensors(lhm_model_loader & ml) = 0; // returns false if cancelled by progress_callback

    // model must define these
    virtual void load_arch_hparams(lhm_model_loader & ml) = 0;
    virtual void load_arch_tensors(lhm_model_loader & ml) = 0;
    virtual std::unique_ptr<llm_graph_context> build_arch_graph(const llm_graph_params & params) const = 0;

protected:
    lhm_model_params params;
...
struct lhm_model_base : public lhm_model {
    friend struct lhm_model;

    lhm_model * model;
    lhm_model_loader * ml = nullptr;
    const LLM_TN tn;

    // lhm_model_loader is not yet defined at this point, so we will set it after construction
    const int TENSOR_DUPLICATED;
    const int TENSOR_NOT_REQUIRED;
    const int TENSOR_SKIP;
    const int TENSOR_SKIP_IF_VIRTUAL;

    explicit lhm_model_base(const lhm_model_params & params);
    virtual ~lhm_model_base() = default;

    ggml_tensor * create_tensor(lhm_model_loader & ml, const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags);

    // convenience overload of create_tensor that doesn't require lhm_model_loader
    ...
    void create_tensor_gate_up_exps(lhm_layer & layer, int bid, int64_t n_embd_,
                int64_t n_ff_, int64_t n_expert_, int flags);

    // helper: try to load merged qkv first, fall back to separate q, k, v
    void create_tensor_qkv(lhm_layer & layer, int bid,
                int64_t n_embd_, int64_t n_embd_q_, int64_t n_embd_k_, int64_t n_embd_v_,
                int flags);

    void load_stats  (lhm_model_loader & ml) override;
    void load_hparams(lhm_model_loader & ml) override;
    void load_vocab  (lhm_model_loader & ml) override;
    bool load_tensors(lhm_model_loader & ml) override;

    // model must define these
    void load_arch_hparams(lhm_model_loader & ml) override = 0;
    void load_arch_tensors(lhm_model_loader & ml) override = 0;
    std::unique_ptr<llm_graph_context> build_arch_graph(const llm_graph_params & params) const override = 0;
};

const char * llm_type_name(llm_type type);

// convenience macro for loading local variables for load_tensors() in lhm_model_base
// note: cast to int64_t since we will use these for the tensor dimensions
#define LHM_LOAD_LOCALS \
