...
lhm_batch_allocr::lhm_batch_allocr(uint32_t n_pos_per_embd) : n_pos_per_embd(n_pos_per_embd) {
    const char * LHM_BATCH_DEBUG = getenv("LHM_BATCH_DEBUG");
    debug = LHM_BATCH_DEBUG ? atoi(LHM_BATCH_DEBUG) : 0;

    seq_pos.resize(LHM_MAX_SEQ);
    seq_cpl.resize(LHM_MAX_SEQ);
    for (auto & cur : seq_cpl) {
        cur.resize(LHM_MAX_SEQ);
    }

    seq_idx.resize(LHM_MAX_SEQ, -1);
}

bool lhm_batch_allocr::init(
        const lhm_batch & batch_inp,
        const lhm_vocab & vocab,
        const lhm_memory_i * memory,
        uint32_t n_embd,
        uint32_t n_seq_max,
        bool output_all) {
    clear();
    ...
    for (int32_t i = 0; i < batch.n_tokens; ++i) {
        const lhm_seq_id s0 = batch.seq_id[i][0];

        for (int32_t s = 0; s < batch.n_seq_id[i]; ++s) {
            const lhm_seq_id s1 = batch.seq_id[i][s];
    ...
    if (n_pos_per_embd > 1) {
        ...
        for (uint32_t s = 0; s < n_seq_max; ++s) {
            ...
            const lhm_pos p0 = memory ? memory->seq_pos_max(s) : -1;
    ...
    //
    {
        seq_set_t cur_seq_set[LHM_MAX_SEQ];
        for (uint32_t s = 0; s < n_seq_max; ++s) {
            cur_seq_set[s].set();
        }

        lhm_pos cur_seq_pos[LHM_MAX_SEQ];
        for (uint32_t s = 0; s < n_seq_max; ++s) {
            cur_seq_pos[s] = -1;
        }

        for (int32_t i = 0; i < batch.n_tokens; ++i) {
            const lhm_pos pos = batch.pos[i];
...
lhm_ubatch lhm_batch_allocr::ubatch_reserve(uint32_t n_seq_tokens, uint32_t n_seqs) {
    ...
    auto udata = std::make_shared<lhm_ubatch::data_t>();
    ...
    udata->seq_idx   .resize(LHM_MAX_SEQ, -1);
...
const lhm_batch & lhm_batch_allocr::get_batch() const {
    return batch;
}

uint32_t lhm_batch_allocr::get_n_tokens() const {
    return batch.n_tokens;
}

uint32_t lhm_batch_allocr::get_n_outputs() const {
    return n_outputs;
}

uint32_t lhm_batch_allocr::get_n_used() const {
    return n_used;
}

std::vector<int32_t> & lhm_batch_allocr::get_out_ids() {
    return out_ids;
}

lhm_pos lhm_batch_allocr::seq_pos_min(lhm_seq_id seq_id) const {
    return seq_pos[seq_id].empty() ? -1 : *seq_pos[seq_id].begin();
}

lhm_pos lhm_batch_allocr::seq_pos_max(lhm_seq_id seq_id) const {
    return seq_pos[seq_id].empty() ? -1 : *seq_pos[seq_id].rbegin();
}

void lhm_batch_allocr::split_reset() {
    out_ids.clear();
...
lhm_ubatch lhm_batch_allocr::split_simple(uint32_t n_ubatch) {
...
lhm_ubatch lhm_batch_allocr::split_seq(uint32_t n_ubatch) {
    // find the first unused token
    uint32_t cur_idx = 0;
    while (cur_idx < used.size() && used[cur_idx]) {
        ++cur_idx;
...
lhm_ubatch lhm_batch_allocr::ubatch_add(const std::vector<int32_t> & idxs, uint32_t n_seqs, bool equal_seqs) {
    ...
    for (size_t i = 0; i < idxs.size(); ++i) {
        ...
        for (int s = 0; s < udata->n_seq_id[i]; ++s) {
            const lhm_seq_id seq_id = batch.seq_id[idxs[i]][s];
    ...
    lhm_seq_id * seq_id_ptr = udata->seq_id_data.data();
    for (size_t i = 0; i < idxs.size(); ++i) {
...
struct lhm_batch lhm_batch_init(int32_t n_tokens_alloc, int32_t embd, int32_t n_seq_max) {
    lhm_batch batch = {
        /*n_tokens =*/ 0,
        /*tokens   =*/ nullptr,
        /*embd     =*/ nullptr,
    ...
    if (embd) {
        ...
        batch.token = (lhm_token *) malloc(sizeof(lhm_token) * n_tokens_alloc);
    }

    batch.pos      = (lhm_pos *)     malloc(sizeof(lhm_pos)      * n_tokens_alloc);
    batch.n_seq_id = (int32_t *)       malloc(sizeof(int32_t)        * n_tokens_alloc);
    batch.seq_id   = (lhm_seq_id **) malloc(sizeof(lhm_seq_id *) * (n_tokens_alloc + 1));
    for (int i = 0; i < n_tokens_alloc; ++i) {
        batch.seq_id[i] = (lhm_seq_id *) malloc(sizeof(lhm_seq_id) * n_seq_max);
    }
