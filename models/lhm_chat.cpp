...
static const std::map<std::string, llm_chat_template> LLM_CHAT_TEMPLATES = {
    ...
    { "lhm2",            LLM_CHAT_TEMPLATE_LHM_2           },
    { "lhm2-sys",        LLM_CHAT_TEMPLATE_LHM_2_SYS       },
    { "lhm2-sys-bos",    LLM_CHAT_TEMPLATE_LHM_2_SYS_BOS   },
    { "lhm2-sys-strip",  LLM_CHAT_TEMPLATE_LHM_2_SYS_STRIP },
...
llm_chat_template llm_chat_detect_template(const std::string & tmpl) {
    ...
    if (tmpl_contains("<|im_start|>")) {
        ...
        if (tmpl_contains("[SYSTEM_PROMPT]")) {
            ...
            // lhm2 template and its variants
            // [variant] support system message
            // See: https://huggingface.co/blog/lhm2#how-to-prompt-lhm_2
            bool support_system_message = tmpl_contains("<<SYS>>");
            bool add_bos_inside_history = tmpl_contains("bos_token + '[INST]");
            bool strip_message = tmpl_contains("content.strip()");
            if (strip_message) {
                return LLM_CHAT_TEMPLATE_LHM_2_SYS_STRIP;
            } else if (add_bos_inside_history) {
                return LLM_CHAT_TEMPLATE_LHM_2_SYS_BOS;
            } else if (support_system_message) {
                return LLM_CHAT_TEMPLATE_LHM_2_SYS;
            } else {
                return LLM_CHAT_TEMPLATE_LHM_2;
            }
        }
        ...
        return LLM_CHAT_TEMPLATE_LHM_3;
    } else if (tmpl_contains("[gMASK]sop")) {
        // chatglm3-6b
...
// Simple version of "lhm_apply_chat_template" that only works with strings
...
int32_t llm_chat_apply_template(
    ...
    if (tmpl == LLM_CHAT_TEMPLATE_CHATML) {
        ...
        for (auto message : chat) {
            ...
            tmpl == LLM_CHAT_TEMPLATE_LHM_2
            || tmpl == LLM_CHAT_TEMPLATE_LHM_2_SYS
            || tmpl == LLM_CHAT_TEMPLATE_LHM_2_SYS_BOS
            || tmpl == LLM_CHAT_TEMPLATE_LHM_2_SYS_STRIP) {
        // lhm2 template and its variants
        // [variant] support system message
        // See: https://huggingface.co/blog/lhm2#how-to-prompt-lhm_2
        bool support_system_message = tmpl != LLM_CHAT_TEMPLATE_LHM_2;
        // [variant] add BOS inside history
        bool add_bos_inside_history = tmpl == LLM_CHAT_TEMPLATE_LHM_2_SYS_BOS;
        // [variant] trim spaces from the input message
        bool strip_message = tmpl == LLM_CHAT_TEMPLATE_LHM_2_SYS_STRIP;
    ...
    } else if (tmpl == LLM_CHAT_TEMPLATE_LHM_3) {
        // Llama 3
        for (auto message : chat) {
            std::string role(message->role);
