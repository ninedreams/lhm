...
class lhm_kv_cache_context;
class lhm_kv_cache_dsa_context;
class lhm_kv_cache_iswa_context;
...
struct lhm_cross {
    ...
    std::vector<std::set<lhm_seq_id>> seq_ids_enc;
};
...
public:
    ...
    virtual void set_input(const lhm_ubatch * ubatch) = 0;
...
// temperature tuning, used by lhm4
...
public:
    llm_graph_input_pos_bucket(const lhm_hparams & hparams) : hparams(hparams) {}
...
public:
    llm_graph_input_pos_bucket_kv(
            const lhm_hparams & hparams,
            const lhm_kv_cache_context * mctx) : hparams(hparams), mctx(mctx) {}
...
public:
    llm_graph_input_mean(const lhm_cparams & cparams) : cparams(cparams) {}
...
public:
    llm_graph_input_cls(const lhm_cparams & cparams, const llm_arch arch) : cparams(cparams), arch(arch) {}
...
public:
    llm_graph_input_rs(const lhm_memory_recurrent_context * mctx) : mctx(mctx) {}
    ...
    const lhm_memory_recurrent_context * mctx;
...
public:
    llm_graph_input_cross_embd(
            const lhm_cross * cross) : cross(cross) {}
    ...
    const lhm_cross * cross;
};

class llm_graph_input_attn_no_cache : public llm_graph_input_i {
public:
    llm_graph_input_attn_no_cache(const lhm_hparams & hparams, const lhm_cparams & cparams) :
        hparams(hparams),
        cparams(cparams) {
    }
...
public:
    llm_graph_input_attn_k_dsa(
            const lhm_hparams & hparams,
            const lhm_cparams & cparams,
            const lhm_kv_cache_dsa_context * mctx) :
        hparams(hparams),
        cparams(cparams),
        mctx(mctx) {
    }
...
public:
    llm_graph_input_attn_kv_iswa(
            const lhm_hparams & hparams,
            const lhm_cparams & cparams,
            const lhm_kv_cache_iswa_context * mctx) :
        hparams(hparams),
        cparams(cparams),
        mctx(mctx) {
    }
...
public:
    llm_graph_input_attn_cross(const lhm_cross * cross) : cross(cross) {}
...
public:
    llm_graph_input_mem_hybrid(
            ...
            const lhm_memory_hybrid_context *      mctx) :
...
public:
    llm_graph_input_mem_hybrid_iswa(
            ...
            std::unique_ptr<llm_graph_input_rs>          inp_rs,
            const lhm_memory_hybrid_iswa_context *     mctx) :
        inp_attn(std::move(inp_attn)),
        inp_rs(std::move(inp_rs)),
        cparams(cparams),
        mctx(mctx) { }
...
public:
    llm_graph_input_sampling(std::map<lhm_seq_id, lhm_sampler *> samplers) :
    ...
    std::map<lhm_seq_id, lhm_sampler *> samplers;
};
...
// these objects deliver the result from the graph build process back to the lhm_context
// note that the input tensors created for the graph are referenced here - the goal is to be able to populate their
//   specific data, by calling the set_inputs() method
// along with the input tensors, the object also provides commonly used outputs tensors, such as logits, embeddings, etc.
//   these are used by the lhm_context to extact the relevant data, based on the compute parameters

// callback that allows us to apply custom logic to each tensor (e.g. ggml-alloc, offloading, etc.)
using llm_graph_cb = std::function<void(const lhm_ubatch & ubatch, ggml_tensor * cur, const char * name, int il)>;
...
struct llm_graph_params {
    ...
    lhm_ubatch ubatch; // note: intentionally make a copy
    ...
    const lhm_adapter_cvec     * cvec;
    const lhm_adapter_loras    * loras;
    ...
    static bool samplers_equal(
          const std::map<lhm_seq_id, lhm_sampler *> & lhs,
          const std::map<lhm_seq_id, lhm_sampler *> & rhs) {
...
public:
    ...
    void set_inputs(const lhm_ubatch * ubatch);
    ...
    std::map<lhm_seq_id, ggml_tensor *> t_sampled_logits;
    std::map<lhm_seq_id, ggml_tensor *> t_candidates;
    std::map<lhm_seq_id, ggml_tensor *> t_sampled;
    std::map<lhm_seq_id, ggml_tensor *> t_sampled_probs;
...
struct llm_graph_context {
    ...
    const enum lhm_pooling_type pooling_type;
    const enum lhm_rope_type    rope_type;
    ...
    // TODO: move this implementation to lhm_memory_recurrent.
    //       this is analogous to lhm_kv_cache::cpy_k / cpy_v
