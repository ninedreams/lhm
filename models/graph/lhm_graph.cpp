...
static ggml_tensor * build_attn_inp_kq_mask(
        ggml_context * ctx,
        const lhm_kv_cache_context * mctx,
...
void llm_graph_input_embd::set_input(const lhm_ubatch * ubatch) {
    if (ubatch->token) {
...
void llm_graph_input_embd_h::set_input(const lhm_ubatch * ubatch) {
    const int64_t n_tokens = ubatch->n_tokens;
...
void llm_graph_input_pos::set_input(const lhm_ubatch * ubatch) {
    if (ubatch->pos && pos) {
        const int64_t n_tokens = ubatch->n_tokens;
        ...
        if (ubatch->token && n_pos_per_embd == 4) {
            ...
            std::vector<lhm_pos> pos_data(n_tokens*n_pos_per_embd);
...
void llm_graph_input_attn_temp::set_input(const lhm_ubatch * ubatch) {
...
void llm_graph_input_pos_bucket::set_input(const lhm_ubatch * ubatch) {
    if (pos_bucket) {
        const int64_t n_tokens = ubatch->n_tokens;
        ...
        for (int j = 0; j < n_tokens; ++j) {
            for (int i = 0; i < n_tokens; ++i) {
                data[j*n_tokens + i] = lhm_relative_position_bucket(ubatch->pos[i], ubatch->pos[j], hparams.n_rel_attn_bkts, true);
            }
        }
    }
}

void llm_graph_input_pos_bucket_kv::set_input(const lhm_ubatch * ubatch) {
    if (pos_bucket) {
...
void llm_graph_input_mean::set_input(const lhm_ubatch * ubatch) {
    if (cparams.embeddings   &&
       (cparams.pooling_type == LHM_POOLING_TYPE_MEAN ||
...
void llm_graph_input_cls::set_input(const lhm_ubatch * ubatch) {
    const int64_t n_tokens     = ubatch->n_tokens;
    const int64_t n_seqs_unq   = ubatch->n_seqs_unq;

    if (cparams.embeddings && (
        cparams.pooling_type == LHM_POOLING_TYPE_CLS  ||
        cparams.pooling_type == LHM_POOLING_TYPE_RANK ||
        ...
        const bool last = (cparams.pooling_type == LHM_POOLING_TYPE_LAST);
...
void llm_graph_input_rs::set_input(const lhm_ubatch * ubatch) {
    GGML_UNUSED(ubatch);
...
bool llm_graph_input_rs::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const lhm_memory_recurrent_context *>(params.mctx);
...
void llm_graph_input_cross_embd::set_input(const lhm_ubatch * ubatch) {
    GGML_UNUSED(ubatch);
...
static void print_mask(const T * data, int64_t n_tokens, int64_t n_kv, int64_t n_swa, lhm_swa_type swa_type) {
    ...
    switch (swa_type) {
        case LHM_SWA_TYPE_NONE:      swa_type_str = "LHM_SWA_TYPE_NONE"; break;
        case LHM_SWA_TYPE_STANDARD:  swa_type_str = "LHM_SWA_TYPE_STANDARD"; break;
        case LHM_SWA_TYPE_CHUNKED:   swa_type_str = "LHM_SWA_TYPE_CHUNKED"; break;
...
void llm_graph_input_attn_no_cache::set_input(const lhm_ubatch * ubatch) {
    const int64_t n_kv     = ubatch->n_tokens;
    const int64_t n_tokens = ubatch->n_tokens;

    const auto fill_mask = [&](auto * data, int64_t ne, int n_swa, lhm_swa_type swa_type) {
        using T = std::remove_reference_t<decltype(*data)>;
        std::fill(data, data + ne, lhm_cast<T>(-INFINITY));

        for (int i1 = 0; i1 < n_tokens; ++i1) {
            const lhm_seq_id s1 = ubatch->seq_id[i1][0];
            const lhm_pos    p1 = ubatch->pos[i1];

            const uint64_t idst = i1*n_kv;

            for (int i0 = 0; i0 < n_tokens; ++i0) {
                const lhm_seq_id s0 = ubatch->seq_id[i0][0];
                const lhm_pos p0    = ubatch->pos[i0];
                ...
                data[idst + i0] = lhm_cast<T>(hparams.use_alibi ? -std::abs(p0 - p1) : 0.0f);
            }
        }
    ...
    if (self_kq_mask->type == GGML_TYPE_F16) {
        ...
        fill_mask((float       *) self_kq_mask->data, ggml_nelements(self_kq_mask), 0, LHM_SWA_TYPE_NONE);
    }
...
void llm_graph_input_attn_kv::set_input(const lhm_ubatch * ubatch) {
...
bool llm_graph_input_attn_kv::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const lhm_kv_cache_context *>(params.mctx);
...
void llm_graph_input_attn_k::set_input(const lhm_ubatch * ubatch) {
    mctx->set_input_k_idxs(self_k_idxs, ubatch);
...
void llm_graph_input_attn_k_dsa::set_input(const lhm_ubatch * ubatch) {
...
bool llm_graph_input_attn_k_dsa::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const lhm_kv_cache_dsa_context *>(params.mctx);
...
void llm_graph_input_attn_kv_iswa::set_input(const lhm_ubatch * ubatch) {
...
bool llm_graph_input_attn_kv_iswa::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const lhm_kv_cache_iswa_context *>(params.mctx);
...
void llm_graph_input_mem_hybrid::set_input(const lhm_ubatch * ubatch) {
...
bool llm_graph_input_mem_hybrid::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const lhm_memory_hybrid_context *>(params.mctx);
...
void llm_graph_input_mem_hybrid_k::set_input(const lhm_ubatch * ubatch) {
    mctx->get_attn()->set_input_k_idxs(inp_attn->self_k_idxs, ubatch);
...
void llm_graph_input_mem_hybrid_iswa::set_input(const lhm_ubatch * ubatch) {
...
bool llm_graph_input_mem_hybrid_iswa::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const lhm_memory_hybrid_iswa_context *>(params.mctx);
...
void llm_graph_input_sampling::set_input(const lhm_ubatch * ubatch) {
    // set the inputs only for the active samplers in the current ubatch
    std::unordered_set<lhm_seq_id> active_samplers;
    for (uint32_t i = 0; i < ubatch->n_tokens; i++) {
        if (ubatch->output[i]) {
            lhm_seq_id seq_id = ubatch->seq_id[i][0];
...
void llm_graph_result::reset() {
    ...
    t_layer_inp.resize(LHM_MAX_LAYERS);
...
void llm_graph_result::set_inputs(const lhm_ubatch * ubatch) {
...
ggml_tensor * llm_graph_context::build_lora_mm(
    ...
    for (const auto & lora : *loras) {
        lhm_adapter_lora_weight * lw = lora.first->get_weight(w);
        if (lw == nullptr) {
            continue;
        }
...
ggml_tensor * llm_graph_context::build_moe_ffn(
     ...
     llm_ffn_op_type   type_op,
                bool   norm_w,
               float   w_scale,
         lhm_expert_gating_func_type gating_op,
                 int   il,
...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    switch (gating_op) {
        case LHM_EXPERT_GATING_FUNC_TYPE_SOFTMAX:
            {
                probs = ggml_soft_max(ctx0, logits); // [n_expert, n_tokens]
            } break;
        case LHM_EXPERT_GATING_FUNC_TYPE_SIGMOID:
            {
    ...
    if (gating_op == LHM_EXPERT_GATING_FUNC_TYPE_SOFTMAX_WEIGHT) {
    ...
    ggml_tensor * cur_experts[LHM_MAX_EXPERTS] = { nullptr };
...
ggml_tensor * llm_graph_context::build_inp_embd(ggml_tensor * tok_embd) const {
    ...
    // token embeddings path (ubatch.token != nullptr)
    {
        ...
        for (const auto & lora : *loras) {
            lhm_adapter_lora_weight * lw = lora.first->get_weight(tok_embd);
            if (lw == nullptr) {
                continue;
            }
...
ggml_tensor * llm_graph_context::build_inp_pos_bucket_dec() const {
    const auto * mctx_cur = static_cast<const lhm_kv_cache_context *>(mctx);
...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        cb(cur, LHM_TENSOR_NAME_FATTN, il);
...
static std::unique_ptr<llm_graph_input_attn_kv> build_attn_inp_kv_impl(
    ...
    const lhm_cparams & cparams,
    const lhm_kv_cache_context * mctx_cur) {
...
llm_graph_input_attn_k_dsa * llm_graph_context::build_attn_inp_k_dsa() const {
    const auto * mctx_cur = static_cast<const lhm_kv_cache_dsa_context *>(mctx);
...
llm_graph_input_attn_kv_iswa * llm_graph_context::build_attn_inp_kv_iswa() const {
    const auto * mctx_cur = static_cast<const lhm_kv_cache_iswa_context *>(mctx);
...
static std::unique_ptr<llm_graph_input_rs> build_rs_inp_impl(
           ggml_context * ctx0,
     const lhm_ubatch & ubatch,
    const lhm_memory_recurrent_context * mctx_cur) {
...
llm_graph_input_rs * llm_graph_context::build_rs_inp() const {
    const auto * mctx_cur = static_cast<const lhm_memory_recurrent_context *>(mctx);
...
llm_graph_input_mem_hybrid * llm_graph_context::build_inp_mem_hybrid() const {
    const auto * mctx_cur = static_cast<const lhm_memory_hybrid_context *>(mctx);
...
llm_graph_input_mem_hybrid_iswa * llm_graph_context::build_inp_mem_hybrid_iswa() const {
    const auto * mctx_cur = static_cast<const lhm_memory_hybrid_iswa_context *>(mctx);
...
void llm_graph_context::build_sampling() const {
    ...
    std::map<lhm_seq_id, int32_t> seq_to_logit_row;
    ...
    // TODO: Call lhm_sampler_accept_ggml after all samplers have been applied.
    /*
    for (const auto & [seq_id, sampler] : samplers) {
        if (auto it = res->t_sampled.find(seq_id); it != res->t_sampled.end()) {
            ggml_tensor * selected_token = it->second;
            if (selected_token != nullptr) {
                lhm_sampler_accept_ggml(sampler, ctx0, gf, selected_token);
            }
        }
    }
    */
...
int32_t lhm_relative_position_bucket(lhm_pos x, lhm_pos y, uint64_t n_buckets, bool bidirectional) {
