...
void lhm_hparams::set_swa_pattern(uint32_t n_pattern, bool dense_first) {
    if (dense_first) {
...
void lhm_hparams::set_recr_pattern(uint32_t n_pattern, bool dense_first) {
    if (dense_first) {
        for (uint32_t il = 0; il < n_layer(); ++il) {
...
bool lhm_hparams::is_swa_any() const {
...
uint32_t lhm_hparams::n_head(uint32_t il) const {
    if (il < n_layer_all) {
...
uint32_t lhm_hparams::n_rot(uint32_t il) const {
    if (il < n_layer_all) {
...
uint32_t lhm_hparams::n_embd_inp_enc() const {
    return n_embd_inp_enc_impl > 0 ? n_embd_inp_enc_impl : n_embd_inp();
}

uint32_t lhm_hparams::n_embd_out() const {
    return n_embd_out_impl > 0 ? n_embd_out_impl : n_embd;
}

uint32_t lhm_hparams::n_embd_head_k(uint32_t il) const {
    if (il < n_layer_all) {
...
uint32_t lhm_hparams::n_embd_v_gqa(uint32_t il) const {
    const uint32_t n_head_kv = this->n_head_kv(il);

    return n_embd_head_v(il) * n_head_kv;
}

bool lhm_hparams::is_n_embd_k_gqa_variable() const {
...
bool lhm_hparams::is_n_embd_v_gqa_variable() const {
...
uint32_t lhm_hparams::n_embd_k_gqa_max() const {
...
uint32_t lhm_hparams::n_embd_v_gqa_max() const {
...
uint32_t lhm_hparams::n_embd_r() const {
    if (wkv_head_size != 0) {
        // for RWKV models
...
uint32_t lhm_hparams::n_embd_s() const {
    if (wkv_head_size != 0) {
...
bool lhm_hparams::is_recr(uint32_t il) const {
    if (il < n_layer_all) {
...
uint32_t lhm_hparams::n_pos_per_embd() const {
    return rope_type == LHM_ROPE_TYPE_MROPE || rope_type == LHM_ROPE_TYPE_IMROPE ? 4 : 1;
}

bool lhm_hparams::is_swa(uint32_t il) const {
    if (il < n_layer_all) {
...
bool lhm_hparams::is_mla() const {
    LHM_ASSERT((n_embd_head_k_mla_impl == 0 && n_embd_head_v_mla_impl == 0) ||
           (n_embd_head_k_mla_impl != 0 && n_embd_head_v_mla_impl != 0));

    return n_embd_head_k_mla_impl != 0 && n_embd_head_v_mla_impl != 0;
}

uint32_t lhm_hparams::n_embd_head_k_mla() const {
    return is_mla() ? n_embd_head_k_mla_impl : n_embd_head_k();
}

uint32_t lhm_hparams::n_embd_head_v_mla() const {
    return is_mla() ? n_embd_head_v_mla_impl : n_embd_head_v();
}

bool lhm_hparams::has_kv(uint32_t il) const {
...
uint32_t lhm_hparams::n_layer() const {
    return n_layer_all - n_layer_nextn;
}

bool lhm_hparams::use_mrope() const {
