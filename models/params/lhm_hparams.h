...
// bump if necessary
#define LHM_MAX_LAYERS  512
#define LHM_MAX_EXPERTS 512 // Qwen3 Next

enum lhm_expert_gating_func_type {
    LHM_EXPERT_GATING_FUNC_TYPE_NONE           = 0,
...
enum lhm_swa_type {
    LHM_SWA_TYPE_NONE      = 0,
    LHM_SWA_TYPE_STANDARD  = 1,
    LHM_SWA_TYPE_CHUNKED   = 2,
    LHM_SWA_TYPE_SYMMETRIC = 3,
};

// forward declaration; full definition in lhm_graph.h
enum llm_ffn_op_type : int;

struct lhm_hparams_posnet {
    uint32_t n_embd;
    uint32_t n_layer;
};

struct lhm_hparams_convnext {
    uint32_t n_embd;
    uint32_t n_layer;
};
...
struct lhm_hparams {
    ...
    // for WavTokenizer
    struct lhm_hparams_posnet   posnet;
    struct lhm_hparams_convnext convnext;

    uint32_t n_shortconv_l_cache  = 0;

    std::array<uint32_t, LHM_MAX_LAYERS> n_head_arr;
    std::array<uint32_t, LHM_MAX_LAYERS> n_head_kv_arr;
    std::array<uint32_t, LHM_MAX_LAYERS> n_ff_arr;
    ...
    uint32_t expert_gating_func   = LHM_EXPERT_GATING_FUNC_TYPE_NONE;
    ...
    lhm_swa_type swa_type = LHM_SWA_TYPE_NONE;
    ...
    std::array<uint32_t, LHM_MAX_LAYERS> is_swa_impl;

    // for hybrid state space models
    std::array<uint32_t, LHM_MAX_LAYERS> is_recr_impl;
    ...
    // xIELU
    std::array<float, LHM_MAX_LAYERS> xielu_alpha_n;
    std::array<float, LHM_MAX_LAYERS> xielu_alpha_p;
    std::array<float, LHM_MAX_LAYERS> xielu_beta;
    std::array<float, LHM_MAX_LAYERS> xielu_eps;
    ...
    enum lhm_pooling_type      pooling_type            = LHM_POOLING_TYPE_NONE;
    enum lhm_rope_type         rope_type               = LHM_ROPE_TYPE_NONE;
    enum lhm_rope_scaling_type rope_scaling_type_train = LHM_ROPE_SCALING_TYPE_NONE;
    ...
    // llm_ffn_op_type_from_string() in lhm_model.cpp, mirroring how
    // rope_scaling_type_train is handled.
    enum llm_ffn_op_type llm_ffn_op;

    // Step35: optional per-layer clamps for (Swi)GLU
    std::array<float, LHM_MAX_LAYERS> swiglu_clamp_exp; // clamping for expert FFN
    std::array<float, LHM_MAX_LAYERS> swiglu_clamp_shexp; // shared expert
    ...
    static bool is_masked_swa(uint32_t n_swa, lhm_swa_type swa_type, lhm_pos p0, lhm_pos p1) {
        LHM_ASSERT(p0 >= 0 && p1 >= 0);

        switch (swa_type) {
            case LHM_SWA_TYPE_NONE:
                {
                } break;
            case LHM_SWA_TYPE_STANDARD:
                {
                    if (p1 - p0 >= (int32_t) n_swa) {
                        return true;
                    }
                } break;
            case LHM_SWA_TYPE_CHUNKED:
                {
                    const lhm_pos pos_chunk_start = (p1 / n_swa) * n_swa;

                    if (p0 < pos_chunk_start) {
                        return true;
                    }
                } break;
            case LHM_SWA_TYPE_SYMMETRIC:
                {
...
static_assert(std::is_trivially_copyable<lhm_hparams>::value, "lhm_hparams must be trivially copyable");
