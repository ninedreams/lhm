...
ggml_tensor * lhm_adapter_cvec::tensor_for(int il) const {
...
ggml_tensor * lhm_adapter_cvec::apply_to(ggml_context * ctx, ggml_tensor * cur, int  il) const {
...
lhm_adapter_lora_weight * lhm_adapter_lora::get_weight(ggml_tensor * w) {
...
static void lhm_adapter_lora_init_impl(lhm_model & model, const char * path_lora, lhm_adapter_lora & adapter) {
    ...
    // check metadata
    {
        ...
        if (kid >= 0) {
            ...
            std::copy(
                (const lhm_token *)data,
                (const lhm_token *)data + seq_len,
    ...
    std::map<std::string, lhm_adapter_lora_weight> ab_map;
    ...
    for (ggml_tensor * cur = ggml_get_first_tensor(ctx.get()); cur; cur = ggml_get_next_tensor(ctx.get(), cur)) {
        ...
        if (str_endswith(name, ".lora_a")) {
            ...
            if (ab_map.find(name) == ab_map.end()) {
                ab_map[name] = lhm_adapter_lora_weight(cur, nullptr);
            } else {
                ab_map[name].a = cur;
            }
            ...
            if (ab_map.find(name) == ab_map.end()) {
                ab_map[name] = lhm_adapter_lora_weight(nullptr, cur);
            } else {
                ab_map[name].b = cur;
            }
    ...
    for (auto & it : ab_map) {
        ...
        lhm_adapter_lora_weight & w = it.second;
        ...
        adapter.ab_map[name] = lhm_adapter_lora_weight(tensor_a, tensor_b);
    }
    ...
    // set tensor data
    {
        lhm_file gguf_file(path_lora, "rb");
...
lhm_adapter_lora * lhm_adapter_lora_init(lhm_model * model, const char * path_lora) {
    lhm_adapter_lora * adapter = new lhm_adapter_lora(model);
...
int32_t lhm_adapter_meta_val_str(const lhm_adapter_lora * adapter, const char * key, char * buf, size_t buf_size) {
...
int32_t lhm_adapter_meta_count(const lhm_adapter_lora * adapter) {
    return (int)adapter->gguf_kv.size();
}

int32_t lhm_adapter_meta_key_by_index(const lhm_adapter_lora * adapter, int i, char * buf, size_t buf_size) {
...
int32_t lhm_adapter_meta_val_str_by_index(const lhm_adapter_lora * adapter, int32_t i, char * buf, size_t buf_size) {
    if (i < 0 || i >= (int)adapter->gguf_kv.size()) {
        if (buf_size > 0) {
            buf[0] = '\0';
        }
...
void lhm_adapter_lora_free(lhm_adapter_lora * adapter) {
    if (adapter == nullptr) {
        return;
    }
