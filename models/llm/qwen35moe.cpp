...
void lhm_model_qwen35moe::load_arch_tensors(lhm_model_loader & ml) {
    LHM_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> lhm_model_qwen35moe::build_arch_graph(const llm_graph_params & params) const {
...
lhm_model_qwen35moe::graph::graph(const lhm_model & model, const llm_graph_params & params) :
...
std::pair<ggml_tensor *, ggml_tensor *> lhm_model_qwen35moe::graph::build_qkvz(
                ggml_tensor * input,
                        int   il) {
...
ggml_tensor * lhm_model_qwen35moe::graph::build_norm_gated(
        ggml_tensor * input,
        ggml_tensor * weights,
        ggml_tensor * gate,
        int           layer) {
...
ggml_tensor * lhm_model_qwen35moe::graph::build_layer_attn(
...
ggml_tensor * lhm_model_qwen35moe::graph::build_layer_attn_linear(
...
ggml_tensor * lhm_model_qwen35moe::graph::build_layer_ffn(ggml_tensor * cur, const int il) {
    ...
    ggml_tensor * moe_out =
        build_moe_ffn(cur,
            ...
            LHM_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il,
...
lhm_model_qwen35moe::graph_mtp::graph_mtp(const lhm_model & model, const llm_graph_params & params)
    : llm_graph_context(params) {
    LHM_ASSERT(hparams.n_layer_nextn > 0 && "QWEN35MOE MTP requires n_layer_nextn > 0");
    LHM_ASSERT(hparams.n_layer_nextn == 1 && "QWEN35MOE MTP currently only supports a single MTP block");
    ...
    LHM_ASSERT(layer.ffn_gate_inp     && "MTP block missing ffn_gate_inp");
    ...
    ggml_tensor * head_norm_w = layer.nextn.shared_head_norm
            ...
            : model.output_norm;
    LHM_ASSERT(head_norm_w && "QWEN35MOE MTP: missing both nextn.shared_head_norm and output_norm");
    ...
    LHM_ASSERT(head_w && "QWEN35MOE MTP: missing LM head (nextn.shared_head_head or model.output)");
