...
void lhm_model_qwen35::load_arch_tensors(lhm_model_loader & ml) {
    LHM_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> lhm_model_qwen35::build_arch_graph(const llm_graph_params & params) const {
...
lhm_model_qwen35::graph::graph(const lhm_model & model, const llm_graph_params & params) :
...
std::pair<ggml_tensor *, ggml_tensor *> lhm_model_qwen35::graph::build_qkvz(
                ggml_tensor * input,
                        int   il) {
...
ggml_tensor * lhm_model_qwen35::graph::build_norm_gated(
        ggml_tensor * input,
        ggml_tensor * weights,
        ggml_tensor * gate,
        int           layer) {
...
ggml_tensor * lhm_model_qwen35::graph::build_layer_attn(
...
ggml_tensor * lhm_model_qwen35::graph::build_layer_attn_linear(
