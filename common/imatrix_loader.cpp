...
static bool common_imatrix_load_legacy(const std::string & fname, common_imatrix & imatrix) {
    ...
    if (!in) {
        LOG_ERROR("failed to open {}", fname.c_str());
        return false;
    }
    ...
    if (in.fail() || n_entries < 1) {
        LOG_ERROR("no data in file {}", fname.c_str());
        return false;
    }
    ...
    for (int i = 0; i < n_entries; ++i) {
        ...
        if (in.fail()) {
            LOG_ERROR("failed reading name for entry {:d} from {}", i + 1, fname.c_str());
            return false;
        }
        name_as_vec[len] = 0;
        ...
        if (in.fail() || nval < 1) {
            LOG_ERROR("failed reading number of values for entry {:d}", i);
            return false;
        }
        ...
        if (in.fail()) {
            LOG_ERROR("failed reading data for entry {:d}", i);
            return false;
        }
...
bool common_imatrix_load(const std::string & fname, common_imatrix & imatrix) {
    ...
    for (const auto & sc : sums_counts_for) {
        ...
        if (!in_sum2 || !counts) {
            LOG_ERROR("mismatched sums and counts for {}", name.c_str());
            gguf_free(ctx_gguf);
            ggml_free(ctx);
            return false;
        }
