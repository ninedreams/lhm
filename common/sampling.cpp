...
// TODO: deduplicate with lhm_impl.h
template<typename T>
struct ring_buffer {
...
struct common_sampler {
    ...
    struct lhm_sampler * grmr;
    struct lhm_sampler * rbudget;
    struct lhm_sampler * chain;
    ...
    void reset() {
        ...
        lhm_sampler_reset(chain);
    }

    void set_logits(struct lhm_context * ctx, int idx) {
...
struct common_sampler * common_sampler_init(const struct lhm_model * model, struct common_params_sampling & params) {
    const lhm_vocab * vocab = lhm_model_get_vocab(model);

    lhm_sampler_chain_params lparams = lhm_sampler_chain_default_params();

    lparams.no_perf = params.no_perf;

    lhm_sampler * grmr = nullptr;
    lhm_sampler * rbudget = nullptr;
    lhm_sampler * chain = lhm_sampler_chain_init(lparams);

    std::vector<lhm_sampler *> samplers;
    ...
    if (grammar_str.compare(0, 11, "%llguidance") == 0) {
        ...
        std::vector<lhm_token> trigger_tokens;
        ...
        if (!grammar_str.empty()) {
             if (params.grammar_lazy) {
                 grmr = lhm_sampler_init_grammar_lazy_patterns(vocab, grammar_str.c_str(), "root",
                         trigger_patterns_c.data(), trigger_patterns_c.size(),
                         trigger_tokens.data(), trigger_tokens.size());
             } else {
                 grmr = lhm_sampler_init_grammar(vocab, grammar_str.c_str(), "root");
             }
        }
    }
    ...
    if (params.has_logit_bias()) {
        samplers.push_back(lhm_sampler_init_logit_bias(lhm_vocab_n_tokens(vocab), params.logit_bias.size(), params.logit_bias.data()));
    }
    ...
    if (params.mirostat == 0) {
                ...
                case COMMON_SAMPLER_TYPE_DRY:
                    {
                        ...
                        samplers.push_back(lhm_sampler_init_dry(vocab, lhm_model_n_ctx_train(model), params.dry_multiplier, params.dry_base, params.dry_allowed_length, params.dry_penalty_last_n, c_breakers.data(), c_breakers.size()));
                    }
                    break;
                case COMMON_SAMPLER_TYPE_TOP_K:
                    samplers.push_back(lhm_sampler_init_top_k(params.top_k));
                    break;
                case COMMON_SAMPLER_TYPE_TOP_P:
                    samplers.push_back(lhm_sampler_init_top_p(params.top_p, params.min_keep));
                    break;
                case COMMON_SAMPLER_TYPE_TOP_N_SIGMA:
                    samplers.push_back(lhm_sampler_init_top_n_sigma(params.top_n_sigma));
                    break;
                case COMMON_SAMPLER_TYPE_MIN_P:
                    samplers.push_back(lhm_sampler_init_min_p(params.min_p, params.min_keep));
                    break;
                case COMMON_SAMPLER_TYPE_XTC:
                    samplers.push_back(lhm_sampler_init_xtc(params.xtc_probability, params.xtc_threshold, params.min_keep, params.seed));
                    break;
                case COMMON_SAMPLER_TYPE_TYPICAL_P:
                    samplers.push_back(lhm_sampler_init_typical(params.typ_p, params.min_keep));
                    break;
                case COMMON_SAMPLER_TYPE_TEMPERATURE:
                    samplers.push_back(lhm_sampler_init_temp_ext(params.temp, params.dynatemp_range, params.dynatemp_exponent));
                    break;
                case COMMON_SAMPLER_TYPE_INFILL:
                    samplers.push_back(lhm_sampler_init_infill(vocab));
                    break;
                case COMMON_SAMPLER_TYPE_PENALTIES:
                    samplers.push_back(lhm_sampler_init_penalties(params.penalty_last_n, params.penalty_repeat, params.penalty_freq, params.penalty_present));
                    break;
        ...
        if (use_adaptive_p) {
            ...
            samplers.push_back(lhm_sampler_init_adaptive_p(params.adaptive_target, params.adaptive_decay, params.seed));
        } else {
            // default: sample from distribution
            samplers.push_back(lhm_sampler_init_dist(params.seed));
        }
    } else if (params.mirostat == 1) {
        samplers.push_back(lhm_sampler_init_temp(params.temp));
        samplers.push_back(lhm_sampler_init_mirostat(lhm_vocab_n_tokens(vocab), params.seed, params.mirostat_tau, params.mirostat_eta, 100));
    ...
    for (auto * smpl : samplers) {
        lhm_sampler_chain_add(chain, smpl);
    }
    ...
    auto * result = new common_sampler {
        ...
        /* .rbudget = */ rbudget,
        /* .chain   = */ chain,
        /* .prev    = */ ring_buffer<lhm_token>(std::max(32, params.n_prev)),
        /* .cur     = */ {},
        /* .cur_p   = */ {},
    };
...
void common_sampler_free(struct common_sampler * gsmpl) {
    ...
    lhm_sampler_free(gsmpl->grmr);
    lhm_sampler_free(gsmpl->rbudget);
    lhm_sampler_free(gsmpl->chain);
...
void common_sampler_accept(struct common_sampler * gsmpl, lhm_token token, bool is_generated) {
    if (!gsmpl) {
        return;
    }
    ...
    if (gsmpl->rbudget && is_generated) {
        lhm_sampler_accept(gsmpl->rbudget, token);
    }

    if (gsmpl->grmr && accept_grammar) {
        lhm_sampler_accept(gsmpl->grmr, token);
    }

    lhm_sampler_accept(gsmpl->chain, token);
...
struct common_sampler * common_sampler_clone(common_sampler * gsmpl) {
    return new common_sampler {
        /* .params  = */ gsmpl->params,
        /* .grmr    = */ lhm_sampler_clone(gsmpl->grmr),
        /* .rbudget = */ lhm_sampler_clone(gsmpl->rbudget),
        /* .chain   = */ lhm_sampler_clone(gsmpl->chain),
        /* .prev    = */ gsmpl->prev,
        /* .cur     = */ gsmpl->cur,
        /* .cur_p   = */ gsmpl->cur_p,
    };
...
void common_perf_print(const struct lhm_context * ctx, const struct common_sampler * gsmpl) {
    // TODO: measure grammar performance

    const double t_sampling_ms = gsmpl ? 1e-3*gsmpl->t_total_us : 0;

    lhm_perf_sampler_data data_smpl;
    lhm_perf_context_data data_ctx;
...
struct lhm_sampler * common_sampler_get(const struct common_sampler * gsmpl) {
    if (!gsmpl) {
        return nullptr;
    }
...
lhm_token common_sampler_sample(struct common_sampler * gsmpl, struct lhm_context * ctx, int idx, bool grammar_first) {
    lhm_synchronize(ctx);

    // start measuring sampling time after the lhm_context synchronization in order to not measure any ongoing async operations
    const auto tm = gsmpl->tm();

    lhm_token id = LHM_TOKEN_NULL;
    ...
    lhm_sampler_apply(rbudget, &cur_p);

    if (grammar_first && grammar_should_apply(gsmpl)) {
        lhm_sampler_apply(grmr, &cur_p);
    }
    ...
    {
        lhm_token_data       single_token_data       = { id, 1.0f, 0.0f };
        lhm_token_data_array single_token_data_array = { &single_token_data, 1, -1, false };

        lhm_sampler_apply(grmr, &single_token_data_array);
...
std::vector<lhm_token> common_sampler_sample_and_accept_n(struct common_sampler * gsmpl, struct lhm_context * ctx, const std::vector<int> & idxs, const lhm_tokens & draft, bool grammar_first) {
    ...
    size_t i = 0;
    for (; i < draft.size(); i++) {
        const lhm_token id = common_sampler_sample(gsmpl, ctx, idxs[i], grammar_first);
...
std::vector<lhm_token> common_sampler_sample_and_accept_n(struct common_sampler * gsmpl, struct lhm_context * ctx, const lhm_tokens & draft, bool grammar_first) {
...
uint32_t common_sampler_get_seed(const struct common_sampler * gsmpl) {
    return lhm_sampler_get_seed(gsmpl->chain);
}
...
lhm_token_data_array * common_sampler_get_candidates(struct common_sampler * gsmpl, bool do_sort) {
    const auto tm = gsmpl->tm();
    ...
    if (do_sort && !res->sorted) {
        ...
        const lhm_token id = res->data[res->selected].id;

        std::sort(res->data, res->data + res->size, [](const lhm_token_data & a, const lhm_token_data & b) {
            return a.p > b.p;
        });
...
lhm_token common_sampler_last(const struct common_sampler * gsmpl) {
...
std::string common_sampler_print(const struct common_sampler * gsmpl) {
    ...
    for (int i = 0; i < lhm_sampler_chain_n(gsmpl->chain); i++) {
        const auto * smpl = lhm_sampler_chain_get(gsmpl->chain, i);
        result += std::string("-> ");
        result += std::string(lhm_sampler_name(smpl)) + " ";
    }

    return result;
}

std::string common_sampler_prev_str(common_sampler * gsmpl, lhm_context * ctx_main, int n) {
