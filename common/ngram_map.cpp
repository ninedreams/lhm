...
static uint32_t common_ngram_map_hash(const lhm_tokens & tokens, size_t start, size_t len) {
    uint32_t hash = 0;
...
// Print the values of a sublist of `lhm_tokens & inp` to a string in the form [v0, v1, v2, ...].
static std::string common_tokens_to_str(const lhm_tokens & inp, size_t start, size_t length) {
 ...
 */
lhm_tokens common_ngram_simple_draft(
        const common_ngram_simple_config & config,
        const lhm_tokens & tokens, lhm_token sampled) {
...
void common_ngram_map_draft(common_ngram_map & map,
        const lhm_tokens & inp, lhm_token sampled,
        lhm_tokens & draft) {
    ...
    std::vector<lhm_token> key_tokens;
