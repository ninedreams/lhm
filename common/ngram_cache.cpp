...
void common_ngram_cache_update(common_ngram_cache & ngram_cache, int ngram_min, int ngram_max,
                              std::vector<lhm_token> & inp, int nnew, bool print_progress) {
...
static lhm_token get_token(const std::vector<lhm_token> & inp, const std::vector<lhm_token> & draft, const size_t i) {
    return i < inp.size() ? inp[i] : draft[1 + i - inp.size()];
}

// If sample size or percentage are below these thresholds the draft is aborted early:
constexpr int    draft_min_sample_size_lax[LHM_NGRAM_MAX] = { 2,  2,  1,  1};
constexpr int        draft_min_percent_lax[LHM_NGRAM_MAX] = {66, 50, 50, 50};
constexpr int draft_min_sample_size_strict[LHM_NGRAM_MAX] = { 4,  3,  2,  2};
constexpr int     draft_min_percent_strict[LHM_NGRAM_MAX] = {75, 66, 66, 66};

// Helper function that tries to draft a token from only the static ngram cache:
static lhm_token try_draft(common_ngram_cache & nc_static, const common_ngram ngram_static) {
    ...
    lhm_token max_token = LHM_TOKEN_NULL;

    for (std::pair<lhm_token, int> token_count_static : part_static) {
        const lhm_token token = token_count_static.first;
    ...
    if (sum_count_static < draft_min_sample_size_lax[LHM_NGRAM_STATIC-1]) {
        return LHM_TOKEN_NULL;
    }
    if (100*max_count_static < draft_min_percent_lax[LHM_NGRAM_STATIC-1]*sum_count_static) {
        return LHM_TOKEN_NULL;
    }
    return max_token;
}
...
static lhm_token try_draft(
    common_ngram_cache & nc_primary, const std::vector<common_ngram> & ngrams_primary, common_ngram_cache_part & part_static,
    const int * min_sample_size, const int * min_percent) {

    lhm_token drafted_token = LHM_TOKEN_NULL;

    for (int i = ngrams_primary.size()-1; i >= 0 && drafted_token == LHM_TOKEN_NULL; --i) {
        ...
        for (std::pair<lhm_token, int> token_count_primary : part_primary) {
            const lhm_token token = token_count_primary.first;
...
void common_ngram_cache_draft(
    ...
    while ((int) draft.size()-1 < n_draft) {
        ...
        const int ngram_start_static = inp_size-LHM_NGRAM_STATIC + draft.size()-1;
        common_ngram ngram_static;
        for (int j = ngram_start_static; j < ngram_start_static + LHM_NGRAM_STATIC; ++j) {
        ...
        if (drafted_token == LHM_TOKEN_NULL) {
