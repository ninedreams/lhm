...
#define LHM_NGRAM_MIN    1
#define LHM_NGRAM_MAX    4
#define LHM_NGRAM_STATIC 2

// Data structures to map n-grams to empirical token probabilities:

struct common_ngram {
    lhm_token tokens[LHM_NGRAM_MAX];

    common_ngram() {
        for (int i = 0; i < LHM_NGRAM_MAX; ++i) {
            tokens[i] = LHM_TOKEN_NULL;
        }
    }

    common_ngram(const lhm_token * input, const int ngram_size) {
        for (int i = 0; i < LHM_NGRAM_MAX; ++i) {
            tokens[i] = i < ngram_size ? input[i] : LHM_TOKEN_NULL;
        }
    }
...
struct common_token_hash_function {
    size_t operator()(const lhm_token token) const {
...
typedef std::unordered_map<lhm_token, int32_t> common_ngram_cache_part;
...
void common_ngram_cache_update(
    common_ngram_cache & ngram_cache, int ngram_min, int ngram_max, std::vector<lhm_token> & inp_data, int nnew, bool print_progress);
