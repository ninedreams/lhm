...
common_preset_context::common_preset_context(lhm_example ex)
...
common_presets common_preset_context::load_from_cache() const {
    ...
    for (const auto & model : cached_models) {
        ...
        preset.set_option(*this, "LHM_ARG_HF_REPO", model.to_string());
        out[preset.name] = preset;
    }
...
common_presets common_preset_context::load_from_models_dir(const std::string & models_dir) const {
    ...
    for (const auto & model : models) {
        ...
        preset.set_option(*this, "LHM_ARG_MODEL", model.path);
        if (!model.path_mmproj.empty()) {
            preset.set_option(*this, "LHM_ARG_MMPROJ", model.path_mmproj);
        }
        out[preset.name] = preset;
    }
