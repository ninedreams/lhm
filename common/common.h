...
enum lhm_example {
    ...
    LHM_EXAMPLE_DEBUG,
    LHM_EXAMPLE_COMMON,
    LHM_EXAMPLE_SPECULATIVE,
    LHM_EXAMPLE_COMPLETION,
    LHM_EXAMPLE_CLI,
    LHM_EXAMPLE_EMBEDDING,
    LHM_EXAMPLE_PERPLEXITY,
    LHM_EXAMPLE_RETRIEVAL,
    LHM_EXAMPLE_PASSKEY,
    LHM_EXAMPLE_IMATRIX,
    LHM_EXAMPLE_BENCH,
    LHM_EXAMPLE_SERVER,
    LHM_EXAMPLE_CVECTOR_GENERATOR,
    LHM_EXAMPLE_EXPORT_LORA,
    LHM_EXAMPLE_MTMD,
    LHM_EXAMPLE_LOOKUP,
    LHM_EXAMPLE_PARALLEL,
    LHM_EXAMPLE_TTS,
    LHM_EXAMPLE_DIFFUSION,
    LHM_EXAMPLE_FINETUNE,
    LHM_EXAMPLE_FIT_PARAMS,
    LHM_EXAMPLE_RESULTS,
    LHM_EXAMPLE_EXPORT_GRAPH_OPS,
...
struct common_grammar_trigger {
    ...
    std::string value;
    lhm_token token = LHM_TOKEN_NULL;
};
...
struct common_params_sampling {
    uint32_t seed = LHM_DEFAULT_SEED; // the seed used to initialize lhm_sampler
    ...
    std::set<lhm_token>               preserved_tokens;

    std::vector<lhm_logit_bias> logit_bias;     // logit biases to apply
    std::vector<lhm_logit_bias> logit_bias_eog; // pre-calculated logit biases for EOG tokens
    ...
    std::vector<lhm_token> reasoning_budget_start;           // start tag token sequence
    std::vector<lhm_token> reasoning_budget_end;             // end tag token sequence
    std::vector<lhm_token> reasoning_budget_forced;          // forced sequence (message + end tag)
...
struct common_params_speculative_draft {
    ...
    std::vector<lhm_model_tensor_buft_override> tensor_buft_overrides;
};
...
struct common_params {
    ...
    std::vector<size_t> fit_params_target = std::vector<size_t>(lhm_max_devices(), 1024 * 1024*1024);

    enum lhm_split_mode split_mode = LHM_SPLIT_MODE_LAYER; // how to split the model across GPUs
    ...
    enum lhm_rope_scaling_type rope_scaling_type = LHM_ROPE_SCALING_TYPE_UNSPECIFIED;
    enum lhm_pooling_type      pooling_type      = LHM_POOLING_TYPE_UNSPECIFIED; // pooling type for embeddings
    enum lhm_attention_type    attention_type    = LHM_ATTENTION_TYPE_UNSPECIFIED; // attention type for embeddings
    enum lhm_flash_attn_type   flash_attn_type   = LHM_FLASH_ATTN_TYPE_AUTO; // whether to use Flash Attention
    ...
    std::vector<lhm_model_kv_override> kv_overrides;
    std::vector<lhm_model_tensor_buft_override> tensor_buft_overrides;

    bool lora_init_without_apply = false; // only load lora to memory, but do not apply it to ctx (user can manually apply lora later using lhm_adapter_lora_apply)
    ...
    bool check             = false; // check rather than generate results for lhm_results
    ...
    lhm_progress_callback load_progress_callback = NULL;
