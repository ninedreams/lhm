...
//   - this function is NOT thread safe because it modifies the global lhm logger state
//   - only parameters that have the same value as in lhm_default_model_params are modified
//     with the exception of the context size which is modified if and only if equal to 0
common_params_fit_status common_fit_params(
                         const char * path_model,
                 lhm_model_params * mparams,
               lhm_context_params * cparams,
                              float * tensor_split,          // writable buffer for tensor split, needs at least lhm_max_devices elements
   lhm_model_tensor_buft_override * tensor_buft_overrides, // writable buffer for overrides, needs at least lhm_max_tensor_buft_overrides elements
...
void common_memory_breakdown_print(const lhm_context * ctx);
