...
static nl::json api_get(const std::string & url,
    ...
    if (is_valid_hf_token(token)) {
        ...
        LOG_WARN("%s: invalid token, authentication disabled\n", __func__);
    }
...
static std::string get_repo_commit(const std::string & repo_id,
    ...
    try {
        ...
        if (!json.is_object() ||
            ...
            LOG_WARN("%s: missing 'branches' for '%s'\n", __func__, repo_id.c_str());
            return {};
        }
        ...
        if (name.empty() || commit.empty()) {
            LOG_WARN("%s: no valid branch for '%s'\n", __func__, repo_id.c_str());
            return {};
        }
...
hf_files get_repo_files(const std::string & repo_id,
    ...
    if (!is_valid_repo_id(repo_id)) {
        LOG_WARN("%s: invalid repository: %s\n", __func__, repo_id.c_str());
        return {};
    }

    std::string commit = get_repo_commit(repo_id, token);
    if (commit.empty()) {
        LOG_WARN("%s: failed to resolve commit for %s\n", __func__, repo_id.c_str());
        return {};
    }
    ...
    try {
        ...
        if (!json.is_array()) {
            LOG_WARN("%s: response is not an array for '%s'\n", __func__, repo_id.c_str());
            return {};
        }
        ...
        for (const auto & item : json) {
            ...
            if (!is_valid_subpath(commit_path, file.path)) {
                LOG_WARN("%s: skip invalid path: %s\n", __func__, file.path.c_str());
                continue;
            }
...
std::string finalize_file(const hf_file & file) {
    ...
    if (!symlinks_disabled.exchange(true)) {
        LOG_WARN("%s: failed to create symlink: %s\n", __func__, ec.message().c_str());
        LOG_WARN("%s: switching to degraded mode\n", __func__);
    }
