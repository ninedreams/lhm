...
#define LHM_MAX_URL_LENGTH 2084 // Maximum URL Length in Chrome: 2083

using json = nlohmann::ordered_json;
using namespace common_arg_utils;

static std::initializer_list<enum lhm_example> mmproj_examples = {
    LHM_EXAMPLE_MTMD,
    LHM_EXAMPLE_SERVER,
    LHM_EXAMPLE_CLI,
};
...
static const std::vector<common_arg> & get_common_arg_defs() {
    static const std::vector<common_arg> options = [] {
        ...
        auto ctx = common_params_parser_init(params, LHM_EXAMPLE_SERVER, nullptr);
        return ctx.options;
    }();
    return options;
}

common_arg & common_arg::set_examples(std::initializer_list<enum lhm_example> examples) {
    this->examples = examples;
    return *this;
}

common_arg & common_arg::set_excludes(std::initializer_list<enum lhm_example> excludes) {
...
bool common_arg::in_example(enum lhm_example ex) {
    return examples.find(ex) != examples.end();
}

bool common_arg::is_exclude(enum lhm_example ex) {
...
bool common_arg::get_value_from_env(std::string & output) const {
    ...
    if (!args_neg.empty()) {
        // for compatibility, we need to check LHM_ARG_NO_ env as well
        std::string neg_env = env;
        string_replace_all(neg_env, "LHM_ARG_", "LHM_ARG_NO_");
...
std::vector<std::string> common_arg::get_env() const {
    ...
    if (!args_neg.empty() && env) {
        // for compatibility, we need to add LHM_ARG_NO_ variant
        std::string neg_env = env;
        string_replace_all(neg_env, "LHM_ARG_", "LHM_ARG_NO_");
...
static void parse_tensor_buffer_overrides(const std::string & value, std::vector<lhm_model_tensor_buft_override> & overrides) {
...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    auto parse_cli_args = [&]() {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (!seen_args.insert(arg).second) {
                ...
                if (!skip) {
                    LOG_WARN("DEPRECATED: argument '%s' specified multiple times, use comma-separated values instead (only last value will be used)\n", arg.c_str());
                }
            }
    ...
    const bool skip_model_download = ctx_arg.ex == LHM_EXAMPLE_EXPORT_GRAPH_OPS;
    ...
    // pad tensor_buft_overrides for lhm_params_fit:
...
static void common_params_print_usage(common_params_context & ctx_arg) {
    ...
    for (auto & opt : ctx_arg.options) {
        // in case multiple LHM_EXAMPLE_* are set, we prioritize the LHM_EXAMPLE_* matching current example
        if (opt.is_sampling) {
    ...
    // TODO: maybe convert enum lhm_example to string
...
static void common_params_print_completion(common_params_context & ctx_arg) {
    ...
    std::set<std::string> executables = {
        ...
        "lhm_cli",
        "lhm_completion",
        "lhm_convert-llama2c-to-ggml",
        "lhm_cvector-generator",
        "lhm_debug",
        "lhm_diffusion-cli",
        "lhm_embedding",
        "lhm_eval-callback",
    ...
    for (const auto& exe : executables) {
        printf("complete -F _lhm_completions %s\n", exe.c_str());
    }
}
...
bool common_params_to_map(int argc, char ** argv, lhm_example ex, std::map<common_arg, std::string> & out_map) {
...
bool common_params_parse(int argc, char ** argv, common_params & params, lhm_example ex, void(*print_usage)(int, char **)) {
...
static std::string list_builtin_chat_templates() {
    ...
    int32_t res = lhm_chat_builtin_templates(nullptr, 0);
    supported_tmpl.resize(res);
    res = lhm_chat_builtin_templates(supported_tmpl.data(), supported_tmpl.size());
    std::ostringstream msg;
...
common_params_context common_params_parser_init(common_params & params, lhm_example ex, void(*print_usage)(int, char **)) {
    // per-example default params
    // we define here to make sure it's included in lhm_gen-docs
    if (ex == LHM_EXAMPLE_COMPLETION) {
        params.use_jinja = false;   // disable jinja by default
    } else if (ex == LHM_EXAMPLE_MTMD) {
        params.use_jinja = false;   // disable jinja by default
        params.sampling.temp = 0.2; // lower temp by default for better quality
    } else if (ex == LHM_EXAMPLE_SERVER) {
     ...
     * rules:
     * - all examples inherit options from LHM_EXAMPLE_COMMON
     * - if LHM_EXAMPLE_* is set (other than COMMON), we only show the option in the corresponding example
     * - if both {LHM_EXAMPLE_COMMON, LHM_EXAMPLE_*,} are set, we will prioritize the LHM_EXAMPLE_* matching current example
     */
    auto add_opt = [&](common_arg arg) {
        if ((arg.in_example(ex) || arg.in_example(LHM_EXAMPLE_COMMON)) && !arg.is_exclude(ex)) {
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_EMBEDDING, LHM_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg(
        {"--display-prompt"},
        {"--no-display-prompt"},
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI}));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_LOOKUP}));
    add_opt(common_arg(
        {"-t", "--threads"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_LOOKUP, LHM_EXAMPLE_SERVER}));
    add_opt(common_arg(
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int value) {
            params.n_ctx = value;
            if (value == 0) {
                // disable context reduction in lhm_params_fit if the user explicitly requests the full context size:
                params.fit_params_min_ctx = UINT32_MAX;
            }
        }
    ).set_env("LHM_ARG_CTX_SIZE"));
    add_opt(common_arg(
        {"-n", "--predict", "--n-predict"}, "N",
        string_format(
            ex == LHM_EXAMPLE_COMPLETION
    ...
    ).set_env("LHM_ARG_CTX_CHECKPOINTS").set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}));
    add_opt(common_arg(
    ...
    ).set_env("LHM_ARG_CHECKPOINT_MIN_SPACING_NT").set_examples({LHM_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"-cram", "--cache-ram"}, "N",
    ...
    ).set_env("LHM_ARG_CACHE_RAM").set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"-kvu", "--kv-unified"},
    ...
    ).set_env("LHM_ARG_KV_UNIFIED").set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_PERPLEXITY, LHM_EXAMPLE_BATCHED, LHM_EXAMPLE_BENCH, LHM_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"--cache-idle-slots"},
        {"--no-cache-idle-slots"},
    ...
    ).set_env("LHM_ARG_CACHE_IDLE_SLOTS").set_examples({LHM_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--context-shift"},
        {"--no-context-shift"},
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_IMATRIX, LHM_EXAMPLE_PERPLEXITY}).set_env("LHM_ARG_CONTEXT_SHIFT"));
    add_opt(common_arg(
        {"--chunks"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_IMATRIX, LHM_EXAMPLE_PERPLEXITY, LHM_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg({ "-fa", "--flash-attn" }, "[on|off|auto]",
                       string_format("set Flash Attention use ('on', 'off', or 'auto', default: '%s')",
                                     lhm_flash_attn_type_name(params.flash_attn_type)),
                       [](common_params & params, const std::string & value) {
                           if (is_truthy(value)) {
    ...
    ).set_excludes({LHM_EXAMPLE_SERVER}));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_DIFFUSION, LHM_EXAMPLE_MTMD}));
    add_opt(common_arg(
        {"--perf"},
        {"--no-perf"},
    ...
    ).set_examples({LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SHOW_TIMINGS"));
    add_opt(common_arg(
        {"-f", "--file"}, "FNAME",
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_DIFFUSION}));
    add_opt(common_arg(
        {"--in-file"}, "FNAME",
    ...
    ).set_examples({LHM_EXAMPLE_IMATRIX}));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION}));
    add_opt(common_arg(
        {"--prompt-cache"}, "FNAME",
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"-sp", "--special"},
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_MTMD, LHM_EXAMPLE_EMBEDDING, LHM_EXAMPLE_RETRIEVAL, LHM_EXAMPLE_PERPLEXITY, LHM_EXAMPLE_DEBUG}));
    add_opt(common_arg(
        {"--spm-infill"},
        string_format(
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--samplers"}, "SAMPLERS",
    ...
    add_opt(common_arg(
        {"-s", "--seed"}, "SEED",
        string_format("RNG seed (default: %d, use random seed for %d)", params.sampling.seed, LHM_DEFAULT_SEED),
        [](common_params & params, const std::string & value) {
    ...
    ).set_sampling().set_env("LHM_ARG_TOP_K"));
    add_opt(common_arg(
        {"--top-p"}, "N",
    ...
    ).set_sampling().set_env("LHM_ARG_BACKEND_SAMPLING"));
    add_opt(common_arg(
        {"--pooling"}, "{none,mean,cls,last,rank}",
        "pooling type for embeddings, use model default if unspecified",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "none") { params.pooling_type = LHM_POOLING_TYPE_NONE; }
            else if (value == "mean") { params.pooling_type = LHM_POOLING_TYPE_MEAN; }
            else if (value == "cls")  { params.pooling_type = LHM_POOLING_TYPE_CLS;  }
            else if (value == "last") { params.pooling_type = LHM_POOLING_TYPE_LAST; }
            else if (value == "rank") { params.pooling_type = LHM_POOLING_TYPE_RANK; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ).set_examples({LHM_EXAMPLE_EMBEDDING, LHM_EXAMPLE_RETRIEVAL, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_DEBUG}).set_env("LHM_ARG_POOLING"));
    add_opt(common_arg(
        {"--attention"}, "{causal,non-causal}",
        "attention type for embeddings, use model default if unspecified",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "causal") { params.attention_type = LHM_ATTENTION_TYPE_CAUSAL; }
            else if (value == "non-causal") { params.attention_type = LHM_ATTENTION_TYPE_NON_CAUSAL; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ).set_examples({LHM_EXAMPLE_EMBEDDING}));
    add_opt(common_arg(
        {"--rope-scaling"}, "{none,linear,yarn}",
        "RoPE frequency scaling method, defaults to linear unless specified by the model",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "none") { params.rope_scaling_type = LHM_ROPE_SCALING_TYPE_NONE; }
            else if (value == "linear") { params.rope_scaling_type = LHM_ROPE_SCALING_TYPE_LINEAR; }
            else if (value == "yarn") { params.rope_scaling_type = LHM_ROPE_SCALING_TYPE_YARN; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ).set_env("LHM_ARG_ROPE_SCALING_TYPE"));
    add_opt(common_arg(
        {"--rope-scale"}, "N",
    ...
    ).set_env("LHM_ARG_ROPE_FREQ_BASE"));
    add_opt(common_arg(
        {"--rope-freq-scale"}, "N",
    ...
    ).set_env("LHM_ARG_ROPE_FREQ_SCALE"));
    add_opt(common_arg(
        {"--yarn-orig-ctx"}, "N",
    ...
    ).set_env("LHM_ARG_YARN_EXT_FACTOR"));
    add_opt(common_arg(
        {"--yarn-attn-factor"}, "N",
    ...
    ).set_env("LHM_ARG_YARN_ATTN_FACTOR"));
    add_opt(common_arg(
        {"--yarn-beta-slow"}, "N",
    ...
    ).set_env("LHM_ARG_YARN_BETA_SLOW"));
    add_opt(common_arg(
        {"--yarn-beta-fast"}, "N",
    ...
    ).set_env("LHM_ARG_YARN_BETA_FAST"));
    add_opt(common_arg(
        {"-gan", "--grp-attn-n"}, "N",
    ...
    ).set_env("LHM_ARG_GRP_ATTN_N").set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_PASSKEY}));
    add_opt(common_arg(
        {"-gaw", "--grp-attn-w"}, "N",
    ...
    ).set_env("LHM_ARG_GRP_ATTN_W").set_examples({LHM_EXAMPLE_COMPLETION}));
    add_opt(common_arg(
        {"-kvo", "--kv-offload"},
        {"-nkvo", "--no-kv-offload"},
    ...
    ).set_examples({LHM_EXAMPLE_PERPLEXITY}));
    add_opt(common_arg(
        {"--hellaswag-tasks"}, "N",
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            GGML_UNUSED(params);
            GGML_UNUSED(value);
            LOG_WARN("DEPRECATED: --defrag-thold is deprecated and no longer necessary to specify\n");
        }
    ).set_env("LHM_ARG_DEFRAG_THOLD"));
    if (ex == LHM_EXAMPLE_SERVER) {
        ...
        ).set_env("LHM_ARG_N_PARALLEL").set_examples({LHM_EXAMPLE_SERVER}));
    } else {
        add_opt(common_arg(
            {"-np", "--parallel"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"-cb", "--cont-batching"},
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_CONT_BATCHING"));
    add_opt(common_arg(
        {"-mm", "--mmproj"}, "FILE",
    ...
    ).set_examples(mmproj_examples).set_env("LHM_ARG_MMPROJ"));
    add_opt(common_arg(
        {"-mmu", "--mmproj-url"}, "URL",
    ...
    ).set_examples(mmproj_examples).set_env("LHM_ARG_MMPROJ_URL"));
    add_opt(common_arg(
        {"--mmproj-auto"},
    ...
    ).set_examples(mmproj_examples).set_env("LHM_ARG_MMPROJ_AUTO"));
    add_opt(common_arg(
        {"--mmproj-offload"},
        {"--no-mmproj-offload"},
    ...
    ).set_examples(mmproj_examples).set_env("LHM_ARG_MMPROJ_OFFLOAD"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_MTMD, LHM_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--image-min-tokens"}, "N",
    ...
    ).set_examples(mmproj_examples).set_env("LHM_ARG_IMAGE_MIN_TOKENS"));
    add_opt(common_arg(
        {"--image-max-tokens"}, "N",
    ...
    ).set_examples(mmproj_examples).set_env("LHM_ARG_IMAGE_MAX_TOKENS"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_MTMD_BATCH_MAX_TOKENS"));
    if (lhm_supports_rpc()) {
        add_opt(common_arg(
            {"--rpc"}, "SERVERS",
    ...
    ).set_env("LHM_ARG_OVERRIDE_TENSOR"));
    add_opt(common_arg(
        {"-cmoe", "--cpu-moe"},
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            if (value == "none") {
                ...
                params.split_mode = LHM_SPLIT_MODE_TENSOR;
            } else {
                throw std::invalid_argument("invalid value");
            }
            if (!lhm_supports_gpu_offload()) {
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (split_arg.size() >= lhm_max_devices()) {
                throw std::invalid_argument(
                    string_format("got %zu input configs, but system only has %zu devices", split_arg.size(), lhm_max_devices())
                );
            }
            for (size_t i = 0; i < lhm_max_devices(); ++i) {
                if (i < split_arg.size()) {
    ...
    ).set_examples({LHM_EXAMPLE_FIT_PARAMS}).set_env("LHM_ARG_FIT_ESTIMATE"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_COMMON, LHM_EXAMPLE_EXPORT_LORA}));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_ALIAS"));
    add_opt(common_arg(
        {"--tags"}, "STRING",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_TAGS"));
    add_opt(common_arg(
        {"-m", "--model"}, "FNAME",
        ex == LHM_EXAMPLE_EXPORT_LORA
            ? "model path from which to load base model"
            : "model path to load",
        [](common_params & params, const std::string & value) {
            params.model.path = value;
        }
    ).set_examples({LHM_EXAMPLE_COMMON, LHM_EXAMPLE_EXPORT_LORA}).set_env("LHM_ARG_MODEL"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg(
        {"--chunk-size"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_PASSKEY, LHM_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"--pos"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_PASSKEY}));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_IMATRIX, LHM_EXAMPLE_CVECTOR_GENERATOR, LHM_EXAMPLE_EXPORT_LORA, LHM_EXAMPLE_TTS, LHM_EXAMPLE_FINETUNE,
                    LHM_EXAMPLE_RESULTS, LHM_EXAMPLE_EXPORT_GRAPH_OPS}));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_BENCH, LHM_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"-tgs"},
    ...
    ).set_examples({LHM_EXAMPLE_BENCH}));
    add_opt(common_arg(
        {"-ntg"}, "n0,n1,...",
    ...
    ).set_examples({LHM_EXAMPLE_EMBEDDING, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_DEBUG}));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_HOST"));
    add_opt(common_arg(
        {"--port"}, "PORT",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_PORT"));
    add_opt(common_arg(
        {"--reuse-port"},
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_REUSE_PORT"));
    add_opt(common_arg(
        {"--path"}, "PATH",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_STATIC_PATH"));
    add_opt(common_arg(
        {"--api-prefix"}, "PREFIX",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_API_PREFIX"));
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_WEBUI_CONFIG"));
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_UI_CONFIG"));
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_WEBUI_CONFIG_FILE"));
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_UI_CONFIG_FILE"));
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_WEBUI_MCP_PROXY"));
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_UI_MCP_PROXY"));
    add_opt(common_arg(
        {"--tools"}, "TOOL1,TOOL2,...",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_TOOLS"));
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_WEBUI"));
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_UI"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_DEBUG}).set_env("LHM_ARG_EMBEDDINGS"));
    add_opt(common_arg(
        {"--rerank", "--reranking"},
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_RERANKING"));
    add_opt(common_arg(
        {"--api-key"}, "KEY",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_API_KEY"));
    add_opt(common_arg(
        {"--api-key-file"}, "FNAME",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_API_KEY_FILE"));
    add_opt(common_arg(
        {"--ssl-key-file"}, "FNAME",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_SSL_KEY_FILE"));
    add_opt(common_arg(
        {"--ssl-cert-file"}, "FNAME",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_SSL_CERT_FILE"));
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            for (const auto & item : parsed.items()) {
                if (item.key() == "enable_thinking") {
                    LOG_WARN("Setting 'enable_thinking' via --chat-template-kwargs is deprecated. "
                            "Use --reasoning on / --reasoning off instead.\n");
                }
                params.default_template_kwargs[item.key()] = item.value().dump();
            }
        }
    ).set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_CHAT_TEMPLATE_KWARGS"));
    add_opt(common_arg(
        {"-to", "--timeout"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_TIMEOUT"));
    add_opt(common_arg(
        {"--sse-ping-interval"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_SSE_PING_INTERVAL"));
    add_opt(common_arg(
        {"--threads-http"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_THREADS_HTTP"));
    add_opt(common_arg(
        {"--cache-prompt"},
        {"--no-cache-prompt"},
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_CACHE_PROMPT"));
    add_opt(common_arg(
        {"--cache-reuse"}, "N",
        string_format(
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_CACHE_REUSE"));
    add_opt(common_arg(
        {"--metrics"},
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_ENDPOINT_METRICS"));
    add_opt(common_arg(
        {"--props"},
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_ENDPOINT_PROPS"));
    add_opt(common_arg(
        {"--slots"},
        {"--no-slots"},
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_ENDPOINT_SLOTS"));
    add_opt(common_arg(
        {"--slot-save-path"}, "PATH",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_MODELS_DIR"));
    add_opt(common_arg(
        {"--models-preset"}, "PATH",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_MODELS_PRESET"));
    add_opt(common_arg(
        {"--models-max"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_MODELS_MAX"));
    add_opt(common_arg(
        {"--models-autoload"},
        {"--no-models-autoload"},
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_MODELS_AUTOLOAD"));
    add_opt(common_arg(
        {"--jinja"},
        {"--no-jinja"},
    ...
    ).set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_MTMD}).set_env("LHM_ARG_JINJA"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_THINK"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_REASONING"));
    add_opt(common_arg(
        {"--reasoning-budget"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_THINK_BUDGET"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_THINK_BUDGET_MESSAGE"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_MTMD}).set_env("LHM_ARG_CHAT_TEMPLATE"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_CHAT_TEMPLATE_FILE"));
    add_opt(common_arg(
        {"--skip-chat-parsing"},
    ...
    ).set_examples({LHM_EXAMPLE_COMPLETION, LHM_EXAMPLE_CLI, LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_SKIP_CHAT_PARSING"));
    add_opt(common_arg(
        {"--prefill-assistant"},
    ...
    ).set_examples({LHM_EXAMPLE_SERVER}).set_env("LHM_ARG_PREFILL_ASSISTANT"));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_CVECTOR_GENERATOR}));
    add_opt(common_arg(
        {"--negative-file"}, "FNAME",
    ...
    ).set_examples({LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}));
    add_opt(common_arg(
    ...
    ).set_env("LHM_ARG_LOG_TIMESTAMPS"));
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SPEC_DRAFT_HF_REPO"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
    ...
    ).set_env("LHM_ARG_SPEC_DRAFT_CACHE_TYPE_K"));
    add_opt(common_arg(
    ...
    ).set_env("LHM_ARG_SPEC_DRAFT_CACHE_TYPE_V"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SPEC_DRAFT_CPU_MOE"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SPEC_DRAFT_N_CPU_MOE"));
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_LOOKUP, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SPEC_DRAFT_N_MAX"));
    add_opt(common_arg(
        {"--spec-draft-n-min"}, "N",
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_LOOKUP, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SPEC_DRAFT_N_MIN"));
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SPEC_DRAFT_P_SPLIT"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SPEC_DRAFT_P_MIN"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SPEC_DRAFT_BACKEND_SAMPLING"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_N_GPU_LAYERS_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SPEC_DRAFT_MODEL"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_SPEC_TYPE"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_LOOKUP, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_DRAFT_MAX"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SPECULATIVE, LHM_EXAMPLE_LOOKUP, LHM_EXAMPLE_SERVER, LHM_EXAMPLE_CLI}).set_env("LHM_ARG_DRAFT_MIN"));
    add_opt(common_arg(
        {"--spec-ngram-size-n"}, "N",
    ...
    ).set_spec().set_examples({LHM_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--spec-ngram-size-m"}, "N",
    ...
    ).set_examples({LHM_EXAMPLE_TTS, LHM_EXAMPLE_SERVER}));
     add_opt(common_arg(
    ...
    ).set_examples({ LHM_EXAMPLE_DIFFUSION }));
    add_opt(common_arg(
        {"--diffusion-visual"},
    ...
    ).set_examples({ LHM_EXAMPLE_FINETUNE }));
    ...
    ).set_examples({LHM_EXAMPLE_RESULTS}));
    add_opt(common_arg(
        {"--save-logits"},
    ...
    ).set_examples({LHM_EXAMPLE_DEBUG}));
    add_opt(common_arg(
    ...
    ).set_examples({LHM_EXAMPLE_EMBEDDING, LHM_EXAMPLE_SERVER}));
