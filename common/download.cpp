    ...
    auto res = cli.Get(resolve_path, headers,
        [&](const httplib::Response &response) {
            if (p.downloaded > 0 && response.status != 206) {
                LOG_WARN("%s: server did not respond with 206 Partial Content for a resume request. Status: %d\n", func, response.status);
                return false;
            }
            if (p.downloaded == 0 && response.status != 200) {
                LOG_WARN("%s: download received non-successful status code: %d\n", func, response.status);
                return false;
            }
            if (p.total == 0 && response.has_header("Content-Length")) {
                try {
                    ...
                    LOG_WARN("%s: invalid Content-Length header: %s\n", func, e.what());
                }
            }
            return true;
        },
...
static int common_download_file_single_online(const std::string & url,
    ...
    if (file_exists && skip_etag) {
        LOG_DEBUG("%s: using cached file: %s\n", __func__, path.c_str());
    ...
    if (file_exists) {
        ...
        LOG_DEBUG("%s: no previous model file found %s\n", __func__, path.c_str());
    }
    ...
    if (head->has_header("Content-Length")) {
        try {
            ...
            LOG_WARN("%s: invalid Content-Length in HEAD response: %s\n", __func__, e.what());
        }
    }
    ...
    if (file_exists) {
        if (etag.empty()) {
            LOG_DEBUG("%s: using cached file (no server etag): %s\n", __func__, path.c_str());
            return 304; // 304 Not Modified - fake cached response
        }
        if (!last_etag.empty() && last_etag == etag) {
            LOG_DEBUG("%s: using cached file (same etag): %s\n", __func__, path.c_str());
            return 304; // 304 Not Modified - fake cached response
        }
    ...
    for (int i = 0; i < max_attempts; ++i) {
        ...
        if (i) {
            LOG_WARN("%s: retrying after %d seconds...\n", __func__, delay);
        ...
        LOG_DEBUG("%s: downloading from %s to %s (etag:%s)...\n",
...
int common_download_file_single(const std::string & url,
    ...
    LOG_DEBUG("%s: using cached file (offline mode): %s\n", __func__, path.c_str());
...
std::string common_docker_resolve_model(const std::string & docker) {
    ...
    try {
        ...
        LOG_DEBUG("%s: Using validated digest: %s\n", __func__, gguf_digest.c_str());
