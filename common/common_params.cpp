...
void fill_common_params(common_params & params) {
    ...
    if (is_set(FLAGS_grammar)) {
        params.sampling.grammar = common_grammar(COMMON_GRAMMAR_TYPE_USER, FLAGS_grammar);
    }

    // // --grammar-file
    // if (is_set(FLAGS_grammar_file)) {
    //     params.sampling.grammar = common_grammar(COMMON_GRAMMAR_TYPE_USER, read_file_contents(FLAGS_grammar_file));
    // }

    // --json-schema
    if (is_set(FLAGS_json_schema)) {
        params.sampling.grammar = common_grammar(COMMON_GRAMMAR_TYPE_OUTPUT_FORMAT, FLAGS_json_schema); // will be converted later
    }

    // --json-schema-file
    if (is_set(FLAGS_json_schema_file)) {
        params.sampling.grammar = common_grammar(COMMON_GRAMMAR_TYPE_OUTPUT_FORMAT, read_file_contents(FLAGS_json_schema_file));
    }
    ...
    if (is_set(FLAGS_cache_type_k) && FLAGS_cache_type_k != "f16") {
        ...
        params.cache_type_k = common_ggml_type_from_name(FLAGS_cache_type_k);
    }

    if (is_set(FLAGS_cache_type_v) && FLAGS_cache_type_v != "f16") {
        params.cache_type_v = common_ggml_type_from_name(FLAGS_cache_type_v);
    }
    ...
    if (is_set(FLAGS_spec_draft_type_k)) {
        params.speculative.draft.cache_type_k = common_ggml_type_from_name(FLAGS_spec_draft_type_k);
    }

    if (is_set(FLAGS_spec_draft_type_v)) {
        params.speculative.draft.cache_type_v = common_ggml_type_from_name(FLAGS_spec_draft_type_v);
    }
